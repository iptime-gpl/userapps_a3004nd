/* $Id: upnpc.c,v 1.5 2013/02/19 08:38:52 thki81 Exp $ */
/* Project : miniupnp
...
#ifdef USE_EFM_PATCH
extern int get_mac_postfix(char *postfix);
struct product_info_s {
	char alias[128];
	char product[128];
	char company[128];
	char homepage[128];
	...
	int lan_num;
	int fan_control;
	int ignore_product_id;
	int hdd_interface_num;
	int fan_status;
	int player;
	int has_esata;
	int esata_slot_id;
...
extern int hwinfo_get_product_code(char *code);
	hwinfo_get_product_code(pd.alias);
#else
extern int get_product_info(struct product_info_s *pd);
	get_product_info(&pd);
#endif
	get_mac_postfix(mac);
	if (description == 0)
		sprintf(desc, "%s_%s", pd.alias, mac);
	else
		sprintf(desc, "%s_%s(%s)", pd.alias, mac, description);
...
#ifdef USE_EFM_PATCH // change delay to 1000.
	if( rootdescurl
	  || (devlist = upnpDiscover(1000, multicastif, minissdpdpath,
		  ...
		  || (i = UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))))
		{
			...
			case 'a':
				...
				SetRedirectAndTest(&urls, &data,
				                   ...
				                   (commandargc > 4)?commandargv[4]:"0", description);
#else
				SetRedirectAndTest(&urls, &data,
				                   commandargv[0], commandargv[1],
				                   commandargv[2], commandargv[3],
			...
			case 'r':
				for(i=0; i<commandargc; i+=2)
				{
					...
					SetRedirectAndTest(&urls, &data,
									   ...
									   commandargv[i], commandargv[i+1], "0", description);
#else
