...
int check_private_ip(char *ifname, unsigned int ip);
#endif
...
syslog_msg( SYSMSG_LOG_INFO, "preform_renew deconfig ..\n");
...
syslog_msg( SYSMSG_LOG_INFO, "perform_release deconfig ..\n");
		run_script(NULL, "deconfig");
	}
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (packet_num < 3) {
					...
					wizard_timeout++;
					if(wizard_timeout > 2 ) 
						wizard_api_set_status("timeout");
#endif

#ifdef USE_SYSTEM_LOG
					if(!syslog_get_flag(10))
					{
						syslog_set_flag(10, 1);
						syslog_msg( SYSMSG_LOG_INFO, SYSLOG_MSG_DHCPC_SERVER_TIMEOUT);
...
syslog_msg( SYSMSG_LOG_INFO, "requesting timeout deconfig ..\n");
...
syslog_msg( SYSMSG_LOG_INFO, "lease lost  deconfig ..\n");
					run_script(NULL, "deconfig");
					timeout = now;
					packet_num = 0;
					change_mode(LISTEN_RAW);
			...
			case INIT_SELECTING:
				...
				if (*message == DHCPOFFER) {
					if ((temp = get_option(&packet, DHCP_SERVER_ID))) {
						...
						if(!check_private_ip(client_config.interface, packet.yiaddr)/* && !check_private_ip(chk_ip) */)
						{
#endif
			...
			case REBINDING:
				if (*message == DHCPACK) {
					...
					if (state != REQUESTING)
					{
syslog_msg( SYSMSG_LOG_INFO, "Received  NAK  deconfig ..\n");
						run_script(NULL, "deconfig");
					}
					state = INIT_SELECTING;
					timeout = now;
...
int check_private_ip(char *ifname, unsigned int serv_address)  
{
	if(!get_dhclient_block_private_ip(ifname))
		return 0;

	fprintf(stderr,"check ip = %08x..\n", serv_address);

	serv_address = htonl(serv_address);

	if( ((serv_address & A_CLASS_PREFIX) == A_CLASS_PRVIATE_IP) || 
	    ((serv_address & B_CLASS_PREFIX) == B_CLASS_PRVIATE_IP) || 
	    ((serv_address & C_CLASS_PREFIX) == C_CLASS_PRVIATE_IP) 
	    )
	{
