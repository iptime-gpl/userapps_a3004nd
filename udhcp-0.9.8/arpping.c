...
int arpping(u_int32_t yiaddr, u_int32_t ip, unsigned char *mac, char *interface, int onlysend)
{
	...
	*((u_int *) arp.tInaddr) = yiaddr;		/* target IP address */
	...
	DEBUG(LOG_ERR,"send_arp ==>");
	if (sendto(s, &arp, sizeof(arp), 0, &addr, sizeof(addr)) < 0)
	{
		fprintf(stderr,"dhclient error 1"); 
#if 0
		rv = 0;
#endif
	}
	...
	while (timeout > 0) {
		...
		if (select(s + 1, &fdset, (fd_set *) NULL, (fd_set *) NULL, &tm) < 0) {
			...
			if (errno != EINTR) 
			{
				fprintf(stderr,"dhclient error 2"); 
#if 0
				rv = 0;
#endif
			}
			...
			if (recv(s, &arp, sizeof(arp), 0) < 0 ) 
			{
				...
				fprintf(stderr,"dhclient error 3"); 
#if 0
				rv = 0;
#endif
			}
