...
"$Id: common.c,v 1.3 2003/06/24 08:18:19 timeprov Exp $";
...
#define TERMINATE_REQUEST_CODE 0x05
#define TERMINATE_ID 0x22
typedef struct lcp_terminate_s {
...
{
    PPPoEPacket packet;
    lcp_terminate_t *l_term = (lcp_terminate_t *)packet.payload;
    int i;
    ...
    packet.session = conn->session;
    packet.length = htons(6);

    l_term->protocol = htons(LCP_PROTOCOL);
    l_term->code = TERMINATE_REQUEST_CODE;
    l_term->id = TERMINATE_ID;
    l_term->length = htons(4);

    plen = sizeof(lcp_terminate_t);
    ...
    syslog(LOG_INFO,"Sent LCP Terminates");
}
#endif
