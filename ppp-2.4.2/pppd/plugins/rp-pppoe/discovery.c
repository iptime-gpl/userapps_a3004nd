...
"$Id: discovery.c,v 1.22 2016/11/29 04:55:56 sgbcmac Exp $";
...
void sendLCPTerminates(PPPoEConnection *conn, int count);

int send_lcp_term(PPPoEConnection *conn, char *ifname)
{
	FILE *fp;
	int ses = 0, i;
	unsigned int mac[6]; 
	char devif[32], buffer[256];

	if((fp=fopen("/etc/pppoe.session", "r")) == NULL ) 
		return 0;

	while(fgets(buffer,256,fp))
	{
		if(!strncmp(buffer,"Id", 2)) continue;

		ses = 0;
		sscanf(buffer, "%08x %02x:%02x:%02x:%02x:%02x:%02x %s\n", &ses,
					&mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5] , devif);
		if(ses && !strcmp(devif, ppp_devnam))
		{
			conn->session = ses;
			for( i = 0 ; i < 6;i++)
				conn->peerEth[i] = (unsigned char)mac[i];
			sendLCPTerminates(conn, 3);
			break;
		}
	}
	fclose(fp);
...
discovery(PPPoEConnection *conn)
{
...
#ifdef SEND_LCP_TERMINATE 
		send_lcp_term(conn, ppp_devnam);
#endif
#ifdef USE_CONFIG_WIZARD
    		set_pppoe_detect_status("wan1","undetected"); /* For Mobile CGI */
		if(wizard_api_get_mode())
		{	
			char wiz_status[32];

			strcpy(wiz_status,"");
			wizard_api_get_status(ppp_devnam, wiz_status);
			if(!strncmp( wiz_status, "detecting",9))
#ifdef USE_NEW_LIB
				set_wan_type( !strcmp(ppp_devnam, IF_WAN) ? WAN1_NAME : WAN2_NAME ,  "dynamic" );	
#ifdef USE_ISYSD
				signal_update();
#endif
#else
				set_external_network_info( !strcmp(ppp_devnam, IF_WAN) ? WAN1_NAME : WAN2_NAME ,  "dynamic" );
#endif
		}
#endif
#ifdef USE_SYSTEM_LOG
		if(!syslog_get_flag(ppp_get_wan_name()))
		{
			syslog_set_flag(ppp_get_wan_name(), 1);
			syslog_msg( SYSMSG_LOG_INFO, "%s( %s [PADI])", SYSLOG_MSG_PPP_NO_REPONSE, get_wan_name(ppp_devnam,1) );
		}
#endif
...
#ifdef USE_CONFIG_WIZARD	
    	set_pppoe_detect_status("wan1","detected"); /* For Mobile CGI */
	if(wizard_api_get_mode()) 
		wizard_api_set_status(ppp_devnam, "detected");
#endif
	...
	if (padrAttempts > MAX_PADI_ATTEMPTS) {
		...
		{
			syslog_set_flag(ppp_get_wan_name(), 1);
			syslog_msg( SYSMSG_LOG_INFO, "%s ( %s [PADS])", SYSLOG_MSG_PPP_NO_REPONSE, get_wan_name(ppp_devnam,1) );
		}
#endif

#ifdef USE_CONFIG_WIZARD	
	if(wizard_api_get_mode()) set_pppoe_status(ppp_devnam, PPPOE_WIZARD_TIMEOUT);
#endif
