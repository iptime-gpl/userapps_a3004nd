 ...
 * Modified for PoPToP
 *
 * $Id: pptpgre.c,v 1.3 2006/08/08 12:36:09 x305 Exp $
 */
...
int decaps_gre(int fd, int (*cb) (int cl, void *pack, unsigned len), int cl)
{
...
#else
	memcpy((char *)&hbuf, (char *)(buffer + ip_len), sizeof(struct pptp_gre_header));
	header = &hbuf; 
#endif
	...
	if (((ntoh8(header->ver) & 0x7F) != PPTP_GRE_VER) ||	/* version should be 1   */
		...
		syslog(LOG_ERR, "     ver [%d] - 0x%02x", ((ntoh8(header->ver) & 0x7F) != PPTP_GRE_VER), (ntoh8(header->ver) & 0x7F));
		syslog(LOG_ERR, "     pro [%d] - 0x%04x(%04x)", (ntoh16(header->protocol) != PPTP_GRE_PROTO), ntoh16(header->protocol), header->protocol);
		syslog(LOG_ERR, "     c-flag [%d]", PPTP_GRE_IS_C(ntoh8(header->flags)));
		syslog(LOG_ERR, "     r-flag [%d]", PPTP_GRE_IS_R(ntoh8(header->flags)));
		syslog(LOG_ERR, "     k-flag [0x%x]", PPTP_GRE_IS_K(ntoh8(header->flags)));
		syslog(LOG_ERR, "     flag [%d], - 0x%02x", (ntoh8(header->flags) & 0xF), ntoh8(header->flags));
		return 0;
	}
