 ...
 *
 * $Id: pptpctrl.c,v 1.8 2012/12/11 10:07:27 ysbcm5358 Exp $
 */
...
#ifdef BCRELAY
static void launch_bcrelay(int parent_pid);
#endif
...
#ifdef USE_OPENTTY2 // EFM patch
	if (openpty2(&pty_fd, &tty_fd, NULL, NULL, NULL) != 0) {
#else
	if (openpty(&pty_fd, &tty_fd, NULL, NULL, NULL) != 0) {
#endif
		syslog(LOG_ERR, "CTRL: openpty() error");
		syslog_perror("openpty");
		syslog_msg(0, "CTRL: openpty() error");
		exit(1);
	}
#endif
	if (pptpctrl_debug) {
...
static void launch_pppd(char **pppaddrs)
{
	...
	pppd_argv[an++] = NULL;
syslog(LOG_DEBUG,"PPPD ARGV %s", pppd_argv[0] );
  ...
  struct dirent *dp;
	...
	syslog(LOG_DEBUG, "CTRL: pppdpid is %s", pppdpid);
...
#else
      {
	      ...
	      for( idx = 0 ;  ; )
	      {
	      	if( !pptpserver_get_index_pptpinfo(idx, &user))
		{
			sleep(3);
			idx = 0;
			continue;
		}
		if(user.pid == parent_pid)
		{
			strcpy(pppif,user.ifname);
			syslog(LOG_DEBUG,"CTRL: PPP IF is %s ", pppif);
			break;
		}
		idx++;
	      }
