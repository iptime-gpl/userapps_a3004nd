...
{ 
	...
	{
		F.mode[DMZTWINIP_MODE_TWINIP].checked = true;
...
function get_hwaddr(wid,hw_addr)        
{ 
        var id; 
        id = 'get_w'+wid+'hosthw';      
...
function addPortForward()
{
        ...
        else if (F.protocol.disabled == false)
        {
                ...
                else if ((F.i_port2.value != '') && (F.i_port1.value > F.i_port2.value))
                {
                ...
                else if ((F.o_port2.value != '') && (F.o_port1.value > F.o_port2.value))
                {
                        alert(NATCONF_PORTFORWARD_INVALID_EXT_PORT_RANGE);
                        F.o_port2.focus();
                        F.o_port2.select();
