...
function ChangeManualSSID(F)
{
	if(!F.wifi_ssid_method) return;
	
	if(F.wifi_ssid_method.value == EXTENDER_MANUAL_SSID_SETUP)
	{
		EnableObj(F.my_ssid_manual);
	
	}
	else if(F.wifi_ssid_method.value == EXTENDER_MAC_SSID_SETUP)
	{
		F.my_ssid_manual.value=F.ssid.value.substr(0,25)+F.macpostfix.value;
		DisableObj(F.my_ssid_manual);
	}
	else if(F.wifi_ssid_method.value == EXTENDER_SAME_SSID_SETUP)
	{
		F.my_ssid_manual.value=F.ssid.value;
		DisableObj(F.my_ssid_manual);
	}
}
...
function SetWirelessForm(doc,F)
{
	...
	descobj=doc.getElementById('extender_desc');
	run=GetValue(F.run);
	...
	if(run == 'extender')
	{
		ShowItDoc(doc,'upper_ap_td');
		ShowItDoc(doc,'my_ap_td');
		HideItDoc(doc,'my_ap_simple_td');
		HideItDoc(doc,'ap_channel_td');
		ChangeManualSSID(F);
		if(descobj) descobj.innerHTML = EXTENDER_DESC_TXT;
		
	}
	else if(run == 'ap')
	{
		HideItDoc(doc,'upper_ap_td');
		HideItDoc(doc,'my_ap_td');
		ShowItDoc(doc,'my_ap_simple_td');
		ShowItDoc(doc,'ap_channel_td');
		if(descobj) descobj.innerHTML = AP_DESC_TXT;
	}
	else if(run == 'client')
	{
		HideItDoc(doc,'my_ap_td');
		...
		HideItDoc(doc,'my_ap_simple_td');
		HideItDoc(doc,'ap_channel_td');
		if(descobj) descobj.innerHTML = CLIENT_DESC_TXT;
	}
...
function CheckWirelessForm(F)
{
	...
	{
                if(F.my_ssid_ap.value == '')
                {
                        alert(MSG_BLANK_SSID_MY_AP);
                        F.my_ssid_ap.focus();
                        return 0;
                }

		if((ssidlen=StrLenUTF8CharCode(F.my_ssid_ap.value)) > 32)
		{
	...
	else 
	{
		...
		if(GetValue(F.wifi_ssid_method) == EXTENDER_MANUAL_SSID_SETUP)
		{
	                if(F.my_ssid_manual.value == '')
       	         	{
                       		alert(MSG_BLANK_SSID_MY_AP);
                        	F.my_ssid_manual.focus();
                        	return 0;
			}
                }
...
function ApplyMBridge(F,act)
{
	...
	if(F.wpapsk_password_view.checked == true) 
		F2.wpapsk_key.value = F.wpapsk_key.value = F.wpapsk_key_text.value;
	else
		F2.wpapsk_key.value = F.wpapsk_key_text.value = F.wpapsk_key.value;
	...
	if(F2.wifi_ssid_method)
		F2.wifi_ssid_method.value = F.wifi_ssid_method.value;
	if(F2.run.value == 'ap')
	{
		F2.my_ssid.value = F.my_ssid_ap.value;
		F2.channel.value = F.channel.value;
		if(F.broadcast_ssid_apmode.checked == true)
			F2.broadcast_ssid.value = 1;
		else
			F2.broadcast_ssid.value = 0;

	}
	else if(F2.run.value == 'extender')
	{
		if(F2.wifi_ssid_method.value == EXTENDER_MANUAL_SSID_SETUP)
			F2.my_ssid.value = F.my_ssid_manual.value;

		if(F.broadcast_ssid_extmode.checked == true)
			F2.broadcast_ssid.value = 1;
		else
			F2.broadcast_ssid.value = 0;
	}
