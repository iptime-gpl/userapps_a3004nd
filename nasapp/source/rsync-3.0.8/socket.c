...
// by thki81 20111206 : void start_accept_loop(int port, int (*fn)(int, int))
#ifdef EFM_PATCH /* by thki81 20111212 */
//void start_accept_loop(int port, int (*fn)(int, int, char*), char *argv)
#else
void start_accept_loop(int port, int (*fn)(int, int), char *argv)
#endif // EFM_PATCH
{
	fd_set deffds;
	int *sp, maxfd, i;
	...
	while (1) {
		...
		if ((pid = fork()) == 0) {
			...
			logfile_reopen();
#ifdef EFM_PATCH /* by thki81 20111206 */
			ret = fn(fd, fd, argv);
#else
			ret = fn(fd, fd);
#endif // EFM_PATCH
