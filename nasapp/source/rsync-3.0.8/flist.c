...
#if defined EFM_PATCH || defined EFM_ROUTER_PATCH // print system time.
			char tmp_buf[256];
			char *addr;
			extern char *auth_user;

			addr = client_addr(NULL); // get current addr.
			sprintf(tmp_buf, "%s(%s)", auth_user, addr);

			if (!strcmp(fbuf, EFM_MAGIC_FILE_SYSTEM_TIME))
			{
				char buf[BIGPATHBUFLEN];
				struct timeval tv;

				gettimeofday(&tv, NULL);
				sprintf(buf, "nas time : %d", (int)tv.tv_sec);
				rwrite(FERROR_XFER, buf, strlen(buf), 0);
			}
#endif // EFM_PATCH || EFM_ROUTER_PATCH
#ifdef EFM_PATCH
			else if (!strcmp(fbuf, EFM_MAGIC_FILE_DOWNLOAD_S))
				efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_DOWNLOAD_START, tmp_buf, "CLOUDBACKUP_SERVICE_DOWNLOAD_START");
			else if (!strcmp(fbuf, EFM_MAGIC_FILE_DOWNLOAD_D))
				efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_DOWNLOAD_START, tmp_buf, "CLOUDBACKUP_SERVICE_DOWNLOAD_DONE");
			else if (!strcmp(fbuf, EFM_MAGIC_FILE_UPLOAD_S))
				efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_UPLOAD_START, tmp_buf, "CLOUDBACKUP_SERVICE_UPLOAD_START");
			else if (!strcmp(fbuf, EFM_MAGIC_FILE_UPLOAD_D))
				efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_UPLOAD_START, tmp_buf, "CLOUDBACKUP_SERVICE_UPLOAD_DONE");
#endif // EFM_PATCH || EFM_ROUTER_PATCH
			continue;
		}
