				...
				else
				{
					...
					if (strlen(EFM_Server_log) > 0)
					{
							...
							strlen(EFM_ipcloud_id_tmp) > 0)
						{
							...
							if (EFM_original)
							{
								sprintf(tmp_buf, "U;%s;%s;%s;", auth_user, client_addr(0), EFM_time_tmp);
							...
							*/
							if (!EFM_original)
							{
								...
								if (EFM_fp)
								{
									fprintf(EFM_fp, "last_sync_time=U%s", EFM_time_tmp);
									...
									//efmlog_simple(EFMLOG_IPCLOUD_SERVER_COMMAND_RUN_DONE, tmp_buf, "IPCLOUD_SERVER_COMMAND_RUN_DONE_U", EFM_ipcloud_id_tmp);
								}
							}
						}
					}
...
#ifdef EFM_PATCH /* 2012-02-23 */
			if (!EFM_original)
			{
				/* delete temp file. */
				char *EFM_pos = NULL;
				char EFM_fname[MAXPATHLEN];
				char EFM_fnametmp[MAXPATHLEN];
				char EFM_command[MAXPATHLEN+16];
				int EFM_off = 0;

				strcpy(EFM_fname, fname);
				EFM_pos = strrchr(EFM_fname, '/');
				if (EFM_pos)
				{
					EFM_off = EFM_pos - EFM_fname + 1; 
					strncpy(EFM_fnametmp, EFM_fname, EFM_off);
					sprintf(EFM_fnametmp+EFM_off, ".%s*", EFM_fname+EFM_off);
					sprintf(EFM_command, "rm -f %s", EFM_fnametmp);
				}
				else
					sprintf(EFM_command, "rm -f .%s*", fname);
