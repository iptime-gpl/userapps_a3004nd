...
#ifdef EFM_PATCH
//	rprintf(FERROR, "###### %s(%d) : modname[%s] modlen(%d)\n", __FUNCTION__, __LINE__, modname, modlen); // EFM_TEST
	io_printf(f_out, "%.*s\n%s\n", modlen, modname, user);
#else
	io_printf(f_out, "%.*s\n", modlen, modname);
#endif // EFM_PATCH
...
#ifdef EFM_PATCH
extern void EFM_write_error_log(void);
static int load_user_config(char *auth_user)
{
	struct stat s;
	char new_config_file[1024];
	sprintf(new_config_file, "/etc/cloudbackup/%s.conf", auth_user);

	// there is not config. 
	if (lstat(new_config_file, &s))
		return 0;

	return lp_user_load(new_config_file);
}
#endif // EFM_PATCH
...
#ifdef EFM_PATCH
	if (EFM_original) // only for cloudbackup.
		load_user_config(auth_user);
#endif // EFM_PATCH
...
#ifdef EFM_PATCH // cloudbackup log
			extern char EFM_fname[MAXPATHLEN];
			if (EFM_original && !dry_run && !list_only)
			{
				char *pos;
				pos = strrchr(request, '/');
				if (pos)
					strcpy(EFM_fname, &pos[1]);
				else
					strcpy(EFM_fname, "");

				char tmp_buf[256];
				sprintf(tmp_buf, "%s(%s)", auth_user, addr);

				// skip new magic file.
				if (strcmp(EFM_fname, EFM_MAGIC_FILE_DOWNLOAD_S) &&
					strcmp(EFM_fname, EFM_MAGIC_FILE_DOWNLOAD_D) &&
					strcmp(EFM_fname, EFM_MAGIC_FILE_UPLOAD_S) &&
					strcmp(EFM_fname, EFM_MAGIC_FILE_UPLOAD_D) &&
					strcmp(EFM_fname, EFM_MAGIC_FILE_SYSTEM_TIME))
				{
					if (!strcmp(EFM_fname, EFM_MAGIC_FILE_DOWNLOAD) || !strcmp(EFM_fname, EFM_MAGIC_FILE_DOWNLOAD1))
						efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_DOWNLOAD_START, tmp_buf, "CLOUDBACKUP_SERVICE_DOWNLOAD_START");
					else if (!strcmp(EFM_fname, EFM_MAGIC_FILE_UPLOAD) || !strcmp(EFM_fname, EFM_MAGIC_FILE_UPLOAD1))
						efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_UPLOAD_START, tmp_buf, "CLOUDBACKUP_SERVICE_UPLOAD_START");
					else
					{
						char tmp_module_id[1024];
						sprintf(tmp_module_id, "%s/", lp_name(module_id));
						if (strcmp(request, tmp_module_id))
						{
							if (am_sender) // download
								efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_DOWNLOAD, tmp_buf, "CLOUDBACKUP_SERVICE_DOWNLOAD", request);
							else
								efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_UPLOAD, tmp_buf, "CLOUDBACKUP_SERVICE_UPLOAD", request);
						}
					}
				}
			}
...
/* this is called when a connection is established to a client
   ...
   here */
// by thki81 20111206 : int start_daemon(int f_in, int f_out)
int start_daemon(int f_in, int f_out, char *argv)
{
	char line[1024];
	char *addr, *host;
	int i;
	 ...
	 * (when isync is run by init and run by a remote shell). */
	if (!load_config(0))
		exit_cleanup(RERR_SYNTAX);
	...
	line[0] = 0;
#ifdef EFM_PATCH
	//	rprintf(FERROR, "###### %s(%d)\n", __FUNCTION__, __LINE__); // EFM_TEST

	if (!read_line_old(f_in, line, sizeof line))
		return -1;

	if (!EFM_original) // for ipcloud
	{
		// read EFM_user
			if (!read_line_old(f_in, EFM_user, sizeof EFM_user))
				return -1;

		//	rprintf(FERROR, "###### %s(%d) line[%s] EFM_user[%s]\n", __FUNCTION__, __LINE__, line, EFM_user); // EFM_TEST
		
		char new_config_file[1024];
		int ret;

		memset(new_config_file, 0, sizeof(new_config_file));
		
		if (strlen(EFM_user) > 0 && strcmp(EFM_user, "root")) // if EFM_user != root
		{
			sprintf(new_config_file, "/etc/rsyncd/%s.conf", EFM_user);

			ret = lp_load(new_config_file, 0);
		
	extern int wait_for_hdd_wakeup(char *wakeup_source);
			wait_for_hdd_wakeup("ipCloud");
		}
		//	rprintf(FERROR, "###### %s(%d) : new_config_file[%s] ret(%d)\n", __FUNCTION__, __LINE__, new_config_file, ret); // EFM_TEST
		//
		
		// change server_id of process
		char *module_name = NULL;
		int tmp_line;

		tmp_line = lp_number(line);
		if (tmp_line >=0)
		{	
			module_name = lp_name(tmp_line);
			//rprintf(FERROR, "========= %s(%d) : module_name[%s]\n", __FUNCTION__, __LINE__, module_name);
		}

		memset(argv, 0, strlen(argv));
		if (module_name)
			sprintf(argv, "--server_id=%s:%s:", EFM_user, module_name);
		else
			sprintf(argv, "--server_id=%s", EFM_user);
		//rprintf(FERROR, "========= %s(%d) : argv[%s]\n", __FUNCTION__, __LINE__, argv);
	}
	else // original
	{
	extern int wait_for_hdd_wakeup(char *wakeup_source);
		wait_for_hdd_wakeup("CloudBackup");
	}
#else
	if (!read_line_old(f_in, line, sizeof line))
		return -1;
...
//by thki81 20111206 : int daemon_main()
int daemon_main(char *argv)
{
	if (is_a_socket(STDIN_FILENO)) {
		...
		// by thki81 20111206 : return start_daemon(STDIN_FILENO, STDIN_FILENO);
		return start_daemon(STDIN_FILENO, STDIN_FILENO, NULL);
	}
	...
	// by thki81 20111206 : start_accept_loop(rsync_port, start_daemon);
	start_accept_loop(rsync_port, start_daemon, argv);
	return -1;
}
