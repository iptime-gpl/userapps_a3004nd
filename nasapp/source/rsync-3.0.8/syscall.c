...
int howmany_char(char *str, char c)
{
        int n,i,len;
...
char *add_slash_char(char *str,char c)
{
        int i,len,j;
        int addlen;
        char *rstr;

        addlen=howmany_char(str,c);
        rstr=malloc(addlen+strlen(str)+4);
...
char *add_slash(char *str, char *search)
{
        int len,i;
        char *rstr,*istr;
        ...
        for( istr=strdup(str),i = 0 ; i < len; i++)
        {
                rstr=add_slash_char(istr,search[i]);
                free(istr);
                istr=rstr;
        }
...
{
#if defined EFM_PATCH || defined EFM_ROUTER_PATCH // by thki81 20111122
extern char *add_slash(char *str, char *search);
#define SPECIAL_CHAR_SH "`$"
	...
	new_fname = add_slash(fname, SPECIAL_CHAR_SH);
	command = malloc(strlen(new_fname) + 20);

	if (command)
	{
		memset(command, 0, strlen(new_fname)+20);
		sprintf(command, "mkdir -p \"%s\"", new_fname);
