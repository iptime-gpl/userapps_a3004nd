...
# endif
...
#if defined EFM_PATCH  || defined EFM_ROUTER_PATCH /* by thki81 2012-02-21 */
#ifdef USE_MV_PLATFORM
	SIGACTMASK(SIGALRM, sig_int);
	if (am_server)
		SIGACTMASK(SIGCHLD, sig_int);
	SIGACTMASK(SIGUSR1, sig_int);
	...
	if (am_server) {
		...
		return daemon_main(orig_argv[2]); // by thki81 20111206 : return daemon_main();
	...
	if (am_server) {
		...
		if (am_daemon)
			return start_daemon(STDIN_FILENO, STDOUT_FILENO, NULL);
			// by thki81 20111206 : return start_daemon(STDIN_FILENO, STDOUT_FILENO);
		start_server(STDIN_FILENO, STDOUT_FILENO, argc, argv);
	}
