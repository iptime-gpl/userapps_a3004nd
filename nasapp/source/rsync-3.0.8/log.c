...
#ifdef EFM_PATCH
char EFM_Server_log[MAXPATHLEN+1024];
...
#ifdef EFM_PATCH // for server log
	if (strlen(buf) > 0)
		strncpy(EFM_Server_log, buf, strlen(buf));
#endif // EFM_PATCH
...
#ifdef EFM_PATCH
unsigned long long int EFM_total_size;
unsigned int EFM_total_cnt;
int EFM_log_flag;
extern char EFM_time[128];
extern char EFM_module_id[128];
extern char EFM_ipcloud_id[128];
#endif // EFM_PATCH
	      ...
	      struct stats *initial_stats, int iflags, const char *hlink)
{
...
#ifdef EFM_PATCH
		if (file->flags & FLAG_LENGTH64) // if file size if bigger that int32
		{
			unsigned long long int tmp_size = OPT_EXTRA(file, 0)->unum;
			//rprintf(FERROR, "##### %s(%d) : 64bit unum[%llu] total_size[%llu]\n", __FUNCTION__, __LINE__, OPT_EXTRA(file, 0)->unum, EFM_total_size);
			EFM_total_size += (tmp_size * 4294967296) + (unsigned long long int)file->len32;
		}
		else
		{
			//rprintf(FERROR, "##### %s(%d) : total_size[%llu]\n", __FUNCTION__, __LINE__, EFM_total_size);
			EFM_total_size += (unsigned long long int)file->len32;
		}
		EFM_total_cnt++;
		//rprintf(FERROR, "##### %s(%d) : total_cnt[%u]\n", __FUNCTION__, __LINE__, EFM_total_cnt);
#endif // EFM_PATCH
	...
	if (code != FCLIENT && logfile_format && *logfile_format) {
		...
		if (strlen(EFM_Server_log) > 0)
		{
			...
			memset(EFM_time, 0, sizeof(EFM_time));
			memset(EFM_module_id, 0, sizeof(EFM_module_id));
			memset(EFM_ipcloud_id, 0, sizeof(EFM_ipcloud_id));
			...
			tok = strtok(tmp_buf, "|");
			if (tok != NULL)
				strncpy(EFM_time, tok, strlen(tok));

			tok = strtok(NULL, "|");
			if (tok != NULL)
				strncpy(EFM_module_id, tok, strlen(tok));

			tok = strtok(NULL, "|");
			if (tok != NULL)
				strncpy(EFM_ipcloud_id, tok, strlen(tok));

			//rprintf(FERROR, "%s(%d) : EFM_time[%s] EFM_module_id[%s] EFM_ipcloud_id[%s]\n", 
			//		__FUNCTION__, __LINE__, EFM_time, EFM_module_id, EFM_ipcloud_id);
				...
				if (!EFM_original)
				{
					...
					if (EFM_fp)
					{
						...
						if (!strcmp(EFM_ipcloud_id, "anonymous"))
							strcpy(tmp_buf, EFM_ipcloud_id);
						else
							sprintf(tmp_buf, "%s.ipdisk.co.kr", EFM_ipcloud_id);

						if (!strcmp(s_or_r, "send"))
						{
							fputs("last_sync_time=NOW_SYNC_D", EFM_fp);

							// logging
							// full_fname(fname);

							if (EFM_log_flag > 0 && !strstr(fname, ".ipcloud_auth"))
								efmlog_simple(EFMLOG_IPCLOUD_SERVER_COMMAND_RUN, tmp_buf, "IPCLOUD_SERVER_COMMAND_RUN", tmp_buf, full_fname(fname));
						}
						else
						{
							fputs("last_sync_time=NOW_UPLOADING", EFM_fp);

							// logging
							// full_fname(fname);
							if (EFM_log_flag > 0)
								efmlog_simple(EFMLOG_IPCLOUD_SERVER_COMMAND_RUN, tmp_buf, "IPCLOUD_SERVER_COMMAND_RUN_U", tmp_buf, full_fname(fname));
						}
						fclose(EFM_fp);
