...
static enum delret delete_item(char *fbuf, uint16 mode, uint16 flags)
{
	...
	if (EFM_original)
	{
		char tmp_user[256], tmp_module[1024];
		sprintf(tmp_user, "%s(%s)", auth_user, client_addr(0));
		sprintf(tmp_module, "%s/%s", lp_name(module_id), fbuf);

		if (!strcmp(fbuf, EFM_MAGIC_FILE_DOWNLOAD) || !strcmp(fbuf, EFM_MAGIC_FILE_DOWNLOAD1))
			efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_DOWNLOAD_DONE, tmp_user, "CLOUDBACKUP_SERVICE_DOWNLOAD_DONE");
		else if (!strcmp(fbuf, EFM_MAGIC_FILE_UPLOAD) || !strcmp(fbuf, EFM_MAGIC_FILE_UPLOAD1))
			efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_UPLOAD_DONE, tmp_user, "CLOUDBACKUP_SERVICE_UPLOAD_DONE");
		else
			efmlog_simple(EFMLOG_CLOUDBACKUP_SERVICE_UPLOAD, tmp_user, "CLOUDBACKUP_SERVICE_UPLOAD_D", tmp_module);
	}
#endif // EFM_PATCH
	enum delret ret;
	char *what;
