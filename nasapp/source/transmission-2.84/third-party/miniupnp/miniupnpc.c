...
tr_logAddError("===============> UNPN Disconver\n");
	if(error)
		*error = UPNPDISCOVER_UNKNOWN_ERROR;
#ifndef USE_EFM_COMMON
...
tr_logAddError("===============> UNPN Disconver return 1\n");
			return devlist;
		}
		deviceIndex++;
	}
...
tr_logAddError("===============> UNPN Disconver return 2\n");
		return NULL;
	}
	/* reception */
...
tr_logAddError("===============> UNPN Disconver 2\n");
#ifdef _WIN32
...
tr_logAddError("===============> UNPN Disconver 3\n");
	if(multicastif)
	{
tr_logAddError("===============> UNPN Disconver multicastif->%s\n",multicastif);
		if(ipv6) {
#if !defined(_WIN32)
...
tr_logAddError("===============> UNPN Disconver 4 --> %08x\n",
...
tr_logAddError("===============> UNPN Disconver 5\n");
	if(error)
		*error = UPNPDISCOVER_SUCCESS;
...
#endif
tr_logAddError("===============> UNPN Disconver 6\n");
#ifdef NO_GETADDRINFO
...
tr_logAddError("===============> before getaddrinfo\n");
...
tr_logAddError("===============> after getaddrinfo\n");
		for(p = servinfo; p; p = p->ai_next) {
tr_logAddError("===============> sendto : p->ai_addr:%d.%d - %d.%d.%d.%d\n",
			p->ai_addr->sa_data[0]&0xff,p->ai_addr->sa_data[1]&0xff,p->ai_addr->sa_data[2]&0xff,
				p->ai_addr->sa_data[3]&0xff,
				p->ai_addr->sa_data[4]&0xff,
				p->ai_addr->sa_data[5]&0xff
				);
	...
	for(state = 1; state <= 3; state++)
	{
		for(dev = devlist; dev; dev = dev->pNext)
		{
			...
			if(descXML)
			{
				...
				free(descXML);
				descXML = NULL;
tr_logAddError("===========> UPNP_GetValidIGD: %s\n", data->CIF.servicetype);
...
tr_logAddError("===========> UPNPIGD_IsConnected: %s\n", urls->controlURL);
#ifdef DEBUG
