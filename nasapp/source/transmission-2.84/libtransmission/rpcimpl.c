 ...
 *
 * $Id: rpcimpl.c,v 1.5 2014/12/19 06:44:46 thki81 Exp $
 */
...
#if 0 // def USE_EFM
      name = tr_utf8clean(file->name, strlen(file->name));
      tr_variantDictAddStr( d, TR_KEY_name, name );
      tr_free(name);
#else
          ...
          const tr_quark           key)
{
  ...
  switch (key)
...
#ifdef USE_EFM
	str = tr_torrentGetDownloadDir( tor );
	str = str+4; // skip '/mnt'.
	
	for (pos = strchr(str, '/');pos;pos = strchr(pos1, '/'))
	{
		pos1 = pos+1;
		if (pos1[0] == '/')
		{
			get_value = 1;
			tr_variantDictAddStr( d, key, pos1);
			break;
		}
	}
	...
	if (!get_value)
      ...
      case TR_KEY_name:
#if 0 //def USE_EFM
	str = tr_utf8clean(tr_torrentName(tor), strlen(tr_torrentName(tor)));
        tr_variantDictAddStr( d, key, str);
	tr_free(str);
#else
  ...
  else
    {
      ...
      if (tr_variantDictFindStr (args_in, TR_KEY_download_dir, &str, NULL))
#ifdef USE_TORRENT_GUI
        {
                ...
                {
                        char new_str[4096];
                        sprintf(new_str, "%s/%s", tr_sessionGetDownloadDir(session), str);
                        tr_ctorSetDownloadDir( ctor, TR_FORCE, new_str);
                }
        }
#else
...
#ifdef EFM_PATCH_NAS101 // only for nas101
    // don't permit to change UP/DOWN alt speed.
#else
...
#ifdef EFM_PATCH_NAS101 // only for nas101
    // don't permit to change peer limit.
#else
...
#ifdef EFM_PATCH_NAS101 // only for nas101
    // don't permit to change UP/DOWN speed.
#else
...
#ifdef USE_TORRENT_GUI
    tr_variantDictAddStr ( d, TR_KEY_incomplete_dir, "/" );
