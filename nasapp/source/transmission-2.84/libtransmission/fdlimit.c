 ...
 *
 * $Id: fdlimit.c,v 1.4 2014/11/19 05:26:35 thki81 Exp $
 */
...
#ifdef USE_EFM_NTFS_FALLOC_PATCH
#define HAVE_POSIX_FALLOCATE

#include <unistd.h>
#include <sys/syscall.h>
int syscall(int number, ...); 
extern int posix_fallocate(int fd, uint64_t offset, uint64_t length);
int posix_fallocate(int fd, uint64_t offset, uint64_t length)
{
	int rc;
	unsigned long len1,len2,off1,off2; 
	

        len1=length>>32;
        len2=length&0xffffffff;

        off1=offset>>32;
        off2=offset&0xffffffff;
        rc = syscall(SYS_fallocate,fd,0,off2,off1,len2,len1);
      	tr_logAddError (_("fallocate system call ret -> %d   length -> %llu errno => %d"), rc, length, errno);
