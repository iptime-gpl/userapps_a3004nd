 ...
 *
 * $Id: rpc-test.c,v 1.1.1.1 2014/09/22 04:10:19 bcmac Exp $
 */
...
printf("test_session_get_and_set()... 1\n");
  session = libttest_session_init (NULL);
printf("test_session_get_and_set()... 2\n");

#if 0
  tor= libttest_zero_torrent_init (session);
printf("test_session_get_and_set()... 3\n");
  check (tor != NULL);
printf("test_session_get_and_set()... 4\n");

  json = "{\"method\":\"session-get\"}";
  tr_rpc_request_exec_json (session, json, strlen(json), rpc_response_func, &response);
printf("test_session_get_and_set()... 5\n");
...
printf("test_session_get_and_set()... 0xff\n");
...
printf("test_session_get_and_set()... return !\n");
  return 0;
}
...
main (void)
{
  ...
  rc = runTests (tests, NUM_TESTS (tests));
printf("rpc-test end.... \n");
  return rc;
}
