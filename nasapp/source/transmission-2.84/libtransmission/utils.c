 ...
 *
 * $Id: utils.c,v 1.3 2015/03/11 12:12:41 bcmac Exp $
 */
...
to_utf8 (const char * in, size_t inlen)
{
  ...
  int i;
#ifdef USE_EFM
  const char * encodings[] = { "EUC-KR", "ISO-8859-15" };
#else
...
#ifdef USE_EFM
#ifdef HAVE_ICONV_OPEN
char *system_characterset(void)
{
        FILE *fp;
        static char sys_charset[24];

        fp = fopen("/etc/sys_charset", "r");
        if (fp)
        {
                int n = 0;
                fgets(sys_charset, 24, fp);
                fclose(fp);
                while(sys_charset[n] != '\0')
                {
                        if (sys_charset[n] == '\n' || sys_charset[n] == '\r')
                                sys_charset[n] = '\0';
                        n++;
...
char*
to_syschar( const char * in, size_t inlen )
{
    char * ret = NULL, *syschar;
    ...
    syschar = system_characterset();

    if (strcmp(syschar, "utf-8"))
    {
        if( tr_utf8_validate( in, inlen, &end  ) )
        {
            iconv_t cd = iconv_open( syschar, "UTF-8" );
            if( cd != (iconv_t)-1 ) {
                if( iconv( cd, &inbuf, &inbytesleft, &outbuf, &outbytesleft ) != (size_t)-1 )
                    ret = tr_strndup( out, buflen-outbytesleft );
                iconv_close( cd );
...
int hex_encode( char *name, char *encode_name)
{
        int i;
        char *p;
        int len = strlen(name);
        ...
        p = encode_name;
        for( i = 0; i < len; i++ )
                p += sprintf(p,"%02x", name[i]);
        return strlen(encode_name);
}

int lock_file(char *property)
{
        int fd;
        char filename[512];

        sprintf(filename,"/var/run/%s.lock", property);
