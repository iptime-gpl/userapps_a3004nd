 ...
 *
 * $Id: session.c,v 1.6 2015/03/11 12:14:54 bcmac Exp $
 */
...
static int get_max_peer_of_system(void)
{
	FILE *fp;
	int max_peers;

	if( (fp=fopen("/var/run/max_peers","r")) == NULL)
		return 0;
	fscanf(fp,"%d", &max_peers);
	fclose(fp);
	return max_peers;
}

int set_peer_limit(tr_session * session, uint16_t n)
{
	int max_peer_of_system;

	max_peer_of_system=get_max_peer_of_system();
tr_logAddError( "===========> max_peers ==> %08x\n", max_peer_of_system);
	if(max_peer_of_system && (max_peer_of_system < n))
  		session->peerLimit = max_peer_of_system;
	else	
  		session->peerLimit = n;
tr_logAddError( "===========> session->peerLimit -> %08x\n", session->peerLimit );
...
accept_incoming_peer (evutil_socket_t fd, short what UNUSED, void * vsession)
{
  ...
  if (clientSocket > 0)
    {
#ifdef USE_EFM // for transmission peer count
#if !defined EFM_ROUTER_PATCH
        increase_transmission_peer_count(NULL);
#endif // EFM_ROUTER_PATCH
#endif //USE_EFM
...
tr_sessionGetDefaultSettings (tr_variant * d)
{
  ...
  tr_variantDictAddBool (d, TR_KEY_peer_port_random_on_start,       true);
#else
...
#ifdef USE_EFM
  tr_variantDictAddInt  (d, TR_KEY_upload_slots_per_torrent,        1);
#else
...
#ifdef USE_EFM // fake daemon
  /*
  tr_quark key_fake_daemon = tr_quark_new ("fake-daemon",  11);
  if (key_fake_daemon)
	  tr_variantDictAddBool( d, key_fake_daemon, false);
*/
#endif // USE_EFM
}
...
#ifdef EFM_ROUTER_PATCH
#define MAX_ROUTER_UPLOAD_LIMIT_K 100
#endif
void
...
#ifdef EFM_ROUTER_PATCH
	if((d == TR_UP) && (toSpeedKBps(Bps) > MAX_ROUTER_UPLOAD_LIMIT_K))
  		s->speedLimit_Bps[d] = toSpeedBytes(MAX_ROUTER_UPLOAD_LIMIT_K);
	else
