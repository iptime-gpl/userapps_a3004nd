 ...
 *
 * $Id: rpc-server.c,v 1.3 2014/11/19 05:26:36 thki81 Exp $
 */
...
#ifdef USE_EFM // for fake daemon
bool fd_flag = false;
#if !defined EFM_ROUTER_PATCH
extern int increase_transmission_peer_count(char *path);
extern int set_hdd_wakeup_flag(int flag, char *wakeup_source);
  ...
  if (req && req->evcon)
    {
      ...
      if (!isAddressAllowed (server, req->remote_host))
        ...
        {
#ifdef USE_EFM // increase peer count.
#if !defined EFM_ROUTER_PATCH
	...
	/*
	    if ((iconfig_get_intvalue("System", "hdd_sleep_op", &EFM_op) != -1 && EFM_op) &&
		(iconfig_get_intvalue("System", "hdd_sleep_type", &EFM_type) != -1 && EFM_type))
	*/
	    if (get_hdd_sleep_status())
		set_hdd_wakeup_flag(1, "Torrent");
#endif // EFM_ROUTER_PATCH
...
#ifdef USE_EFM // for fake daemon
  key = tr_quark_new("fake-daemon", 11);
  if (key)
    tr_variantDictFindBool( settings, key, &fd_flag);
#endif // USE_EFM
