 ...
 *
 * $Id: cache.c,v 1.2 2014/11/04 02:11:39 thki81 Exp $
 */
...
#ifdef USE_EFM
      const int cacheCutoff = 1 + cache->max_blocks / 2;
#else
...
#ifdef USE_EFM
#define MAX_CACHE_BLOCKS (0x1400000/MAX_BLOCK_SIZE)
#endif
int
                    ...
                    struct evbuffer  * writeme)
{
  ...
  if (cb == NULL)
    {
#ifdef USE_EFM
    	    if( tr_ptrArraySize( &cache->blocks ) > MAX_CACHE_BLOCKS )
	    {
    			cacheTrim( cache );
			return 0;
	    }
