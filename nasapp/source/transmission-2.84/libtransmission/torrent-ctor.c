 ...
 *
 * $Id: torrent-ctor.c,v 1.3 2014/12/19 05:52:20 thki81 Exp $
 */
...
#define TORRENT_LIST_LOCK "torrent.list.lock"
#define TORRENT_WAIT_LIST_FILE "/tmp/torrent-wait.list"

static int add_torrent_wait_list(char *value)
{
        FILE *fp;
        int fd, rc = 1;
        char key[1024];
        ...
        fd = lock_file(TORRENT_LIST_LOCK);

        hex_encode(value, key);
        key[520] = '\0'; // Max key length 520 bytes

        if (rc)
        {
                fp = fopen(TORRENT_WAIT_LIST_FILE, "a");
                if (fp)
                {
                        fprintf(fp, "wait%s;%s;\n", key, value);
                        fclose(fp);
...
int
tr_ctorSetMetainfoFromWatchFile( tr_ctor *    ctor,
                            const char * filename )
{
    uint8_t * metainfo;
    size_t    len;
    ...
    if( ctor->isSet_metainfo )
    {
	    ...
	    {
		char *tmp;
		tmp = tr_utf8clean(name, -1);
		add_torrent_wait_list(tmp);
		tr_free(tmp);
	    }
        }
    }
