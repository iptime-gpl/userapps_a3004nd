 ...
 *
 * $Id: daemon.c,v 1.4 2014/12/19 02:44:06 thki81 Exp $
 */
...
#ifdef USE_EFM
static tr_quark key_fake_daemon;
#endif // USE_EFM
...
#ifdef USE_EFM
    { 's', "speed-limit-down", "Max download speed", "s", 1, "<limit>" },
    { 'S', "speed-limit-up", "Max upload speed", "S", 1, "<limit>" },
    { 'A', "no-allowed", "Disable Allowed IP addresses. (Default: " TR_DEFAULT_RPC_WHITELIST ")", "A", 0, NULL },
#endif
...
#ifdef USE_EFM// chroot patch
    { 'R', "chroot", "Set the root directory", "R", 1, "<dir>" },
    { 'U', "user", "Set user for change privileges", "U", 1, "<user>" },
    { 'G', "group", "Set group for change privileges", "G", 1, "<group>" },
    { 920, "fake-daemon", "enable fake daemon mode", "fd", 0, NULL },
    { 921, "no-fake-daemon", "disable fake daemon mode", "FD", 0, NULL },
#endif // USE_EFM
    { 953, "global-seedratio", "All torrents, unless overridden by a per-torrent setting, should seed until a specific ratio", "gsr", 1, "ratio" },
    { 954, "no-global-seedratio", "All torrents, unless overridden by a per-torrent setting, should seed regardless of ratio", "GSR", 0, NULL },
    { 'x', "pid-file", "Enable PID file", "x", 1, "<pid-file>" },
#ifdef USE_EFM
    { 'X', "message-level", "Message Level", "X", 1, "<level>" },
#endif
...
#ifdef USE_EFM // chroot patch
static int  
SetupPrivileges( int argc, const char ** argv )  
{  
    int c;  
    int ret = 1;
    ...
    {  
        ...
        ent_pw = getpwnam( user );  
        if ( !ent_pw )  
        {  
            fprintf( stderr, "failed to get user info: %s\n", tr_strerror( errno ) );  
            return 0;  
        }  
        pw_uid = ent_pw->pw_uid;  
        pw_gid = ent_pw->pw_gid;  
    }  
  
    if ( group )  
    {  
        struct group *ent_gp;  
        ent_gp = getgrnam( group );  
        if ( !ent_gp )  
        {  
            fprintf( stderr, "failed to get group info: %s\n", tr_strerror( errno ) );  
            return 0;  
        }  
        pw_gid = ent_gp->gr_gid;  
    }  
  
    if ( ( user || group ) && ( setgid( pw_gid ) < 0 ) )  
    {  
    ...
    {  
        ...
        {  
            fprintf( stderr, "failed to initgroups: %s\n", tr_strerror( errno ) );  
            return 0;  
        }  
        if ( setuid( pw_uid ) < 0 )  
        {  
...
onFileAdded (tr_session * session, const char * dir, const char * file)
{
...
#ifdef USE_EFM
    int err = tr_ctorSetMetainfoFromWatchFile (ctor, filename);
#else
    ...
    {
            char * new_filename = tr_strdup_printf( "%s.invalid", filename );
            rename( filename, new_filename );
            tr_free( new_filename );

            tr_logAddError( "Invaild torrent file \"%s\"", file );
    }
#endif
...
#ifdef USE_EFM // chroot patch
    if (!SetupPrivileges(argc, (const char**)argv))
	    exit(1);
#endif // USE_EFM
    ...
    while ((c = tr_getopt (getUsage (), argc, (const char**)argv, options, &optarg))) {
        switch (c) {
            ...
            case 'p': tr_variantDictAddInt (&settings, TR_KEY_rpc_port, atoi (optarg));
                      break;
#ifdef USE_EFM
            case 's': tr_variantDictAddInt( &settings, TR_KEY_speed_limit_down, atoi( optarg ) );
		      tr_variantDictAddBool( &settings, TR_KEY_speed_limit_down_enabled, (atoi(optarg)) ? true : false );
                      break;
            case 'S': tr_variantDictAddInt( &settings, TR_KEY_speed_limit_up, atoi( optarg ) );
		      tr_variantDictAddBool( &settings, TR_KEY_speed_limit_up_enabled, (atoi(optarg)) ? true : false );
                      break;
            case 'A': tr_variantDictAddBool( &settings, TR_KEY_rpc_whitelist_enabled, false );
                      break;
#endif
            ...
            case 'r': tr_variantDictAddStr (&settings, TR_KEY_rpc_bind_address, optarg);
                      break;
#ifdef USE_EFM // chroot patch, fake daemon
            case 'R': /* handled above */
            case 'U':
            case 'G':
		      break;
            case 920: // enable fake daemon
		      key_fake_daemon = tr_quark_new ("fake-daemon",  11);
                      tr_variantDictAddBool( &settings, key_fake_daemon, true );
            case 921: // disable fake daemon
		      key_fake_daemon = tr_quark_new ("fake-daemon",  11);
                      tr_variantDictAddBool( &settings, key_fake_daemon, false );
		      break;
#endif // USE_EFM
            ...
            case 'x': tr_variantDictAddStr (&settings, key_pidfile, optarg);
                      break;
#ifdef USE_EFM
	    case 'X': tr_variantDictAddInt( &settings,  TR_KEY_message_level, 
			(atoi(optarg) == 1) ? TR_LOG_ERROR : (atoi(optarg) == 2) ? TR_LOG_INFO : TR_LOG_DEBUG);
                      break;
#endif
