 ...
 *
 * $Id: remote.c,v 1.4 2014/11/18 09:15:13 thki81 Exp $
 */
...
#ifdef USE_EFM // for supporting remote gui.
int GUI_Support;
#endif // USE_EFM
...
{
#if defined USE_EFM && !defined EFM_ROUTER_PATCH // ysyoo, EFM
    char locale[12];
    ...
    {
        if (!strncmp(locale, "kr", 2))  
	{
		if ((eta / (60 *60)) > 0)
        		tr_snprintf( buf, buflen, "%2" PRId64 "시간" "%2" PRId64 "분", eta / ( 60 * 60 ), (eta / 60) % 60);
		else if ((eta / 60) % 60 > 0)
        		tr_snprintf( buf, buflen, "%2" PRId64 "분", (eta / 60) % 60);
		else
        		tr_snprintf( buf, buflen, "%2" PRId64 "초", eta);
	}
	else
	{
        	tr_snprintf( buf, buflen, "%02" PRId64 ":" "%02" PRId64 ":" "%02" PRId64, 
    ...
    {
        if (!strncmp(locale, "kr", 2))  
        	tr_snprintf( buf, buflen, "%" PRId64 " 일", eta / ( 60 * 60 * 24 ) );
	else
...
{
    if (bytes < 0)
#ifdef USE_EFM // ysyoo, EFM
        tr_strlcpy (buf, "-- --", buflen);
#else
...
#ifdef USE_EFM // add new option for supporting gui.
    { 'z', "remote-gui",             "Enable remote gui", "z",  0, NULL },
    { 'Z', "no-remote-gui",          "Disable remote gui", "Z",  0, NULL },
#endif // USE_EFM
...
getStatusString (tr_variant * t, char * buf, size_t buflen)
{
    ...
    else switch (status)
    {
        ...
        case TR_STATUS_SEED_WAIT:
            tr_strlcpy (buf, "Verify", buflen);
            break;
        case TR_STATUS_STOPPED:
            if (tr_variantDictFindBool (t, TR_KEY_isFinished, &boolVal) && boolVal)
                tr_strlcpy (buf, "Done", buflen);
            else
                tr_strlcpy (buf, "Stop", buflen);
            break;
        ...
        case TR_STATUS_SEED: {
            ...
            tr_variantDictFindInt (t, TR_KEY_peersSendingToUs, &toUs);
                ...
                if (leftUntilDone > 0)
                    tr_strlcpy (buf, "Down", buflen);
                else
		{
#if defined EFM_ROUTER_PATCH 
		    tr_strlcpy( buf, "Up", buflen );
#else // nas
		    int seeding = 0;
		    if (iconfig_get_intvalue("Torrent", "seeding", &seeding) == -1)
			    tr_strlcpy( buf, "Done", buflen );
		    else
		    {
			    if (seeding)
...
#endif // USE_EFM
{
      ...
      && (tr_variantDictFindList (args, TR_KEY_torrents, &torrents)))
    {
...
#if defined USE_EFM && !defined EFM_ROUTER_PATCH // ysyoo, EFM
        {
		int64_t      id, eta, status, up, down, elapsed;
		int64_t      sizeWhenDone, leftUntilDone, totalSize;
		double       ratio;
		const char * name;
		tr_variant *    files;

		tr_variantDictFindInt( t, TR_KEY_id, &id );
            	tr_variantDictFindStr( t, TR_KEY_hashString, &str, NULL );
			printf("%d_%s;", (int)id, str);
		tr_variantDictFindStr( t, TR_KEY_name, &name, NULL );
			printf("%s;", to_syschar( name, strlen(name)));
		tr_variantDictFindInt( t, TR_KEY_sizeWhenDone, &sizeWhenDone );
			strlsize( buf, sizeWhenDone, sizeof( buf ) );
			printf("%s;", buf);
		tr_variantDictFindInt( t, TR_KEY_leftUntilDone, &leftUntilDone );
		if (sizeWhenDone)
		{
			printf("%d%%;", (int)( 100.0 * ( sizeWhenDone - leftUntilDone ) / sizeWhenDone ));
			printf("%s;", getStatusString( t, buf, sizeof( buf )));
		}
		else
		{
			printf("0%;Down;");
		}
		tr_variantDictFindInt( t, TR_KEY_rateDownload, &down );
			printf("%3.0f KB/s;", down/(double)tr_speed_K);
#ifdef USE_EFM
			int seeding = 0;
			if (iconfig_get_intvalue("Torrent", "seeding", &seeding) == -1)
				printf("0 KB/s;");
			else
			{
				if (seeding)
				{
					tr_variantDictFindInt( t, TR_KEY_rateUpload, &up );
						printf("%3.0f KB/s;", up/(double)tr_speed_K);
				}
				else
					printf("0 KB/s;");
			}
		...
		tr_variantDictFindInt( t, TR_KEY_rateUpload, &up );
			printf("%3.0f KB/s;", up/(double)tr_speed_K);
#endif // USE_EFM
		tr_variantDictFindInt( t, TR_KEY_addedDate, &i );
		tr_variantDictFindInt( t, TR_KEY_doneDate, &j );
		if (!j) tr_variantDictFindInt( t, TR_KEY_activityDate, &j );
			if ( (j-i) > 0 )
				etaToString( buf, sizeof( buf ), j-i );
			else
				strcpy( buf, "-- -- " );
			printf("%s;", buf);
		tr_variantDictFindInt( t, TR_KEY_eta, &eta );
			if( !sizeWhenDone || leftUntilDone || eta != -1 )
				etaToString( buf, sizeof( buf ), eta );
			else
			{
				char locale[12];
				get_system_locale(locale);
			...
			printf("%s;", buf);
		tr_variantDictFindStr( t, TR_KEY_downloadDir, &str, NULL );
		{
#ifdef USE_EFM // for supporting remote gui.
			if (GUI_Support)
				printf("%s;", str);
			else
			{
				...
				printf("%s;", to_syschar(&ptr[1], strlen(&ptr[1])));
			}
#else
			char *ptr = strchr(&str[1], '/');
			printf("%s;", to_syschar(&ptr[1], strlen(&ptr[1])));
		...
		tr_variantDictFindInt( t, TR_KEY_downloadedEver, &i );
			strlsize( buf, i, sizeof( buf ) );
			printf( "%s;", buf );
		tr_variantDictFindInt( t, TR_KEY_uploadedEver, &j );
			strlsize( buf, j, sizeof( buf ) );
			printf( "%s;", buf );
			printf("0;0;"); // current seed / total seed

		tr_variantDictFindInt( t, TR_KEY_peersSendingToUs, &j ); 
			printf("%" PRId64 ";", j);
		tr_variantDictFindInt( t, TR_KEY_peersConnected, &i );
			printf("%" PRId64 ";", i);
        ...
        for (i = 0, in = tr_variantListSize (torrents); i < in; ++i)
        {
              ...
              && tr_variantDictFindList (d, TR_KEY_wanted, &wanteds))
            {
                int j = 0, jn = tr_variantListSize (files);
                    ...
                    int64_t      length;
                    const char * filename;
		    int pages = (jn/LIST_PER_PAGE) + ((jn%LIST_PER_PAGE) ? 1 : 0);
		printf("%d;\n", pages);
                ...
                for( j = 0; j < jn; ++j )
		{
                      ...
                      && tr_variantDictFindInt( file, TR_KEY_bytesCompleted, &have ))
		    {
			    char    *ptr = strrchr(filename, '/');
                            ...
                            char     havestr[64], lenstr[64];

                            strlsize( havestr, have, sizeof( havestr ) );
                            strlsize( lenstr, length, sizeof( lenstr ) );

			    printf("%s;%3.0f%%;%s;%s\n",
			    	(ptr) ? to_syschar(&ptr[1], strlen(&ptr[1])) : 
					to_syschar(filename, strlen(filename)),
				floor( 100.0 * percent ), havestr, lenstr);
		    }
		}
#else
                printf ("%s (%d files):\n", name, jn);
...
flush (const char * rpcurl, tr_variant ** benc)
{
    ...
    if (cmd_retry && res)
    {
	while ((cmd_retry <= 10) && res )
	{
	    printf("transmission-remote cmd retry %d \n", cmd_retry);
    	    res = curl_easy_perform( curl );
	    cmd_retry++;
	}
    }	
    ...
    if( res )
    {
        tr_logAddNamedError (MY_NAME, "(%s) %s", rpcurl, curl_easy_strerror( res ) );
        status |= EXIT_FAILURE;
	printf("transmission-remote error\n");
	exit(1);
    }
#else
