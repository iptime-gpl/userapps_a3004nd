...
_get_flctags(char *filename, struct song_metadata *psong)
{
	...
	{
#ifdef USE_EFM_PATCH
		/* ysyoo, EFM, 2011-09-15 */
		if(iterator) 
		{
			FLAC__metadata_simple_iterator_delete(iterator);
			DPRINTF(E_ERROR, L_SCANNER, "Free metadata buffer from %s\n", filename);
		}
#endif
...
#ifdef USE_EFM_PATCH
			if (!psong->image) /* ysyoo, EFM, 2011-09-15 */
#endif
			{
				DPRINTF(E_WARN, L_SCANNER, "METADATA_TYPE_PICTURE  [%s]\n", filename);
