...
check_for_captions(const char * path, sqlite_int64 detailID)
{
...
#ifdef EFM_SMI_SUPPORT /* by thki82 2012-03-09 */
        char tmp_file[PATH_MAX];
	char caption_ext[20][8] = {".smi", ".Smi", ".sMi", ".smI", ".SMi", ".SmI", ".sMI", ".SMI", 
	...
	for (i=0; i<16; i++)
	{
		...
		sql_exec(db, "INSERT into CAPTIONS"
			" (ID, PATH, EXT) "
			"VALUES"
			" (%lld, %Q, %Q)", detailID, tmp_file, &caption_ext[i][1]);
		break;
	}
#else
	strcat(file, ".srt");
