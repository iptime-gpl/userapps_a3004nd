...
int http_request_parse(server *srv, connection *con) 
{
...
#ifdef EFM_CHUNK_PATCH
							else if (cmp > 0 && 0 == (cmp = buffer_caseless_compare(CONST_BUF_LEN(ds->key), CONST_STR_LEN("Transfer-Encoding")))) 
							{
								if (ds->value->used == 0) SEGFAULT();

								//syslog(LOG_DEBUG,"Transfer-Encoding: is %s", ds->value->ptr);
								if(!strcmp(ds->value->ptr,"Chunked"))
									con->request.chunk_encoding = 1;
							}
							else if (cmp > 0 && 0 == (cmp = buffer_caseless_compare(CONST_BUF_LEN(ds->key), CONST_STR_LEN("X-Expected-Entity-Length")))) 
							{
								char *err;
								unsigned long long r;
								size_t j;
								...
								r = atoll(ds->value->ptr);
								con->request.expected_entity_length = r;
							}
#endif
	...
	if (con_length_set) {
		...
		if (con->request.content_length != 0) {
			// syslog(LOG_DEBUG,"CONTENT LENGTH is OK....");
			return 1;
		}
	}
