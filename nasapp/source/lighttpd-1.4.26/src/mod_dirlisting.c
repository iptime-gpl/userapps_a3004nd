...
#ifdef EFM_PATCH
int check_this_uri_root(char *ptr)
{
//	syslog(LOG_DEBUG,"uri -> %s", ptr);
	ptr=strchr(&ptr[1], '/');
	if(!ptr) return 1;
	ptr=strchr(&ptr[1], '/');
//	syslog(LOG_DEBUG,"----> ptr: %s", ptr);
...
static void http_list_directory_header(server *srv, connection *con, plugin_data *p, buffer *out) {
	...
	if (p->conf.auto_layout) {
		...
		if (p->conf.external_css->used > 1) {
			...
			buffer_append_string_len(out, CONST_STR_LEN(
...
#ifdef EFM_PATCH
				"h2 {margin-bottom: 6px; font-family:arial; font-size:10pt}\n"
#else
				...
				"}\n"
#ifdef EFM_PATCH
				"td {padding-right: 14px; padding-bottom:3px; font-family:arial; font-size: 9pt;}\n"
#else
...
#ifdef EFM_PATCH
	{
		...
		next_dirptr=strchr(&con->uri.path->ptr[1],'/');
		...
		if(next_dirptr)
		{
			...
			//next_dirptr++;
			tbuffer=buffer_init_string(next_dirptr);
			buffer_append_string_encoded(out, CONST_BUF_LEN(tbuffer), ENCODING_MINIMAL_XML);
			buffer_free(tbuffer);
		}
		else
			buffer_append_string_encoded(out, CONST_BUF_LEN(con->uri.path), ENCODING_MINIMAL_XML);
...
#ifdef EFM_PATCH
	if(!check_this_uri_root(con->uri.path->ptr))
#endif
	buffer_append_string_len(out, CONST_STR_LEN(
		"<tr>"
...
extern httpd_conf_t hc;

int DIRLIST_EFMLOG( connection *con , char *log_userid, int flag , int public)
{
	char *filenameptr;
	int dirflag = (con->uri.path->ptr[con->uri.path->used - 2] == '/')?1:0;

	filenameptr = strchr( &con->uri.path->ptr[1], '/');
	if(!filenameptr)
		return 0;
	if(flag)
	{
		if (dirflag)
			efmlog_simple( public?EFMLOG_DIRLIST_PUBLIC_FOLDER_ACCESS_ACCEPT:EFMLOG_DIRLIST_FOLDER_ACCESS_ACCEPT, 
					log_userid, 
					public?"DIRLIST_PUBLIC_FOLDER_ACCESS_ACCEPT":"DIRLIST_FOLDER_ACCESS_ACCEPT", 
					filenameptr); 
		else
			efmlog_simple( public?EFMLOG_DIRLIST_PUBLC_FILE_GET:EFMLOG_DIRLIST_FILE_GET, 
					log_userid, 
					public?"DIRLIST_PUBLC_FILE_GET":"DIRLIST_FILE_GET", filenameptr);
	}
	else
	{
		if (dirflag)
			efmlog_simple( EFMLOG_DIRLIST_FOLDER_ACCESS_DENY, log_userid, "DIRLIST_FOLDER_ACCESS_DENY", filenameptr);
		else
			efmlog_simple( EFMLOG_DIRLIST_FILE_GET_DENY, log_userid, "DIRLIST_FILE_GET_DENY", filenameptr); 
	}
	return 0;
}


int redirect_to_dirlogin(connection *con, server *srv)
{
	buffer *out;

	out = chunkqueue_get_append_buffer(con->write_queue);
	buffer_append_string_len(out, CONST_STR_LEN("<script> location = \"/dirlogin.cgi\"; </script>"));
	response_header_overwrite(srv, con, CONST_STR_LEN("Content-Type"), CONST_STR_LEN("text/html"));
	// syslog(LOG_DEBUG,"out- >%s", out->ptr);
	con->file_finished = 1;
...
URIHANDLER_FUNC(mod_dirlisting_subrequest) {
	...
	char *ptr;
	data_string *ds;
	char remoteip[32], *cookie;
	char session_id[128], log_userid[128];
#endif
	...
	sprintf(log_userid,"%s",remoteip);

	//syslog(LOG_DEBUG,"mod_dirlist -----> <%s> ", con->uri.path->ptr);
	if(!strncmp(con->uri.path->ptr,"/httpstream",strlen("/httpstream")))
	{
		syslog(LOG_DEBUG,"http stream no auth Now ");
		/* TBD */
	}
	else if(!strncmp(hc.dirlist_tag, &con->uri.path->ptr[1], strlen(hc.dirlist_tag)))
	{
		char userid[64];

		// syslog(LOG_DEBUG,"diraccess:normal -----> <%s> ", con->uri.path->ptr);
		ds = (data_string *)array_get_element(con->request.headers, "Cookie");
		if(!ds)
		{
			redirect_to_dirlogin(con, srv);
			DIRLIST_EFMLOG(con,log_userid,0, 0);
		...
		if(!check_http_session("DirList",session_id,remoteip))
		{
			redirect_to_dirlogin(con, srv);
			DIRLIST_EFMLOG(con, log_userid, 0, 0);
		...
		update_http_session("DirList", session_id);

		if(get_user_by_session("DirList", session_id, userid))
			sprintf(log_userid,"%s(%s)",userid,remoteip);
		DIRLIST_EFMLOG(con, log_userid, 1, 0);
	}
	else if( strcmp(hc.dirlist_public_tag,"") && !strncmp(hc.dirlist_public_tag, &con->uri.path->ptr[1], strlen(hc.dirlist_public_tag)))
	{
		char locale[32];
		get_system_locale(locale);
		// syslog(LOG_DEBUG,"diraccess:public -> %s ", con->uri.path->ptr );
		sprintf(log_userid,"%s(%s)",GET_COMMON_TEXT(locale,"PublicUser"),remoteip);

		DIRLIST_EFMLOG(con, log_userid, 1, 1);
	}
#endif
