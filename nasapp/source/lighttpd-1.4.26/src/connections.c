...
found_header_end:
	...
	case CON_STATE_READ_POST:
		...
		//syslog(LOG_DEBUG,"Read POST: %d", con->request.http_method);
#ifdef EFM_CHUNK_PATCH
		if( con->request.http_method == HTTP_METHOD_PUT && con->request.content_length == 0 && con->request.chunk_encoding )
		{
			//syslog(LOG_DEBUG,"Read Chunk Encoding: %d Expected Length: %llu, %08x", con->request.chunk_encoding, con->request.expected_entity_length, cq->first);
                
			for (c = cq->first; c && (dst_cq->bytes_in != (off_t)con->request.expected_entity_length) ; c = c->next) 
			{
				off_t weHave, toRead;
                        
				assert(c->mem->used);
                        
				toRead = weHave = c->mem->used - c->offset - 1;
                        
				//syslog(LOG_DEBUG,"HTTP_METHOD_PUT toRead: %d", toRead);
                                
				davh=get_webdav_handle(con);
				if(!davh) davh=open_webdav_handle(con);

				if(!davh || (toRead != webdav_write_httpchunk(con,davh,c->mem->ptr+c->offset,toRead)))
				{
					/* write failed for some reason ... disk full ? */
					syslog(LOG_DEBUG,"write failed: 405 response(%08x,toRead:%llu",(int)davh,toRead);
					con->http_status = 409; /* Request-Entity too large */
					connection_set_state(srv, con, CON_STATE_HANDLE_REQUEST);
					break;
				}
                                
				c->offset += toRead;
				if(davh->writesize >= davh->filesize)
				{
					connection_set_state(srv, con, CON_STATE_HANDLE_REQUEST);
					close_webdav_handle(davh);
					//syslog(LOG_DEBUG,"Last WebDAV write --> close davh ..");
					break;
				}
			}
		}
		...
		//syslog(LOG_DEBUG,"HTTP_METHOD_PUT toRead: %d", toRead);
				...
				if(!davh || (toRead != webdav_write_direct(con,davh,c->mem->ptr+c->offset,toRead)))
				{
					/* write failed for some reason ... disk full ? */
					//syslog(LOG_DEBUG,"write failed: 405 response(%08x,toRead:%d",davh,toRead);
					con->http_status = 409; /* Request-Entity too large */
					connection_set_state(srv, con, CON_STATE_HANDLE_REQUEST);
					break;
				}
			...
			if (con->request.content_length > 64 * 1024) {
				...
				if( (con->request.http_method == HTTP_METHOD_POST) 
				     && ((exph=get_exp_handle(con)) || (exph=open_exp_handle(con,c->mem->ptr+c->offset)))
				  )
				{
					if(toRead != exp_write_upload(exph,c->mem->ptr+c->offset,toRead))
					{
						/* write failed for some reason ... disk full ? */
						log_error_write(srv, __FILE__, __LINE__, "sbs",
...
#ifdef EFM_UPLOAD
				exp_handle_t tmp_exph;
				if(extract_session_exp_id(con,tmp_exph.session,tmp_exph.exp_id) &&
					parse_upload_mulipart_header(con, c->mem->ptr+c->offset, &tmp_exph.w_offset, &tmp_exph.filename, &tmp_exph.filesize))
				{
					tmp_exph.writesize = tmp_exph.filesize;
					update_exp_upload_status(&tmp_exph);
				}     
#endif // EFM_UPLOAD
			}
