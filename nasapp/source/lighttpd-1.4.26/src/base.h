...
typedef struct {
	...
	/* CONTENT */
#ifdef EFM_PATCH
	/* size_t is 32bit, off_t is 64bit , for upload size */
...
typedef struct webdav_handle_s {
        struct webdav_handle_s *next;
        char ip[32];
        char *uri;
        int fd;
...
#ifdef EFM_CHUNK_PATCH
	int chunk_size;  /* include cr/lf */
	int chunk_offset;
	int chunk_status;
#define HTTP_CHUNK_STATUS_READ_SIZE 0x0
#define HTTP_CHUNK_STATUS_READ_DATA 0x1
#define HTTP_CHUNK_STATUS_DATA_COMPLETE 0x2
	unsigned char *chunk_data;
#define MAX_CHUNK_SIZE_STR 32
	unsigned char chunksize_buf[MAX_CHUNK_SIZE_STR];
	int chunksize_offset;
#define MAX_HTTP_CHUNK		0x100000
#endif
