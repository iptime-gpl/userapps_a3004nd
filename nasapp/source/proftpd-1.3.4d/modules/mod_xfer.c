 ...
 *
 * $Id: mod_xfer.c,v 1.3 2016/05/16 07:16:12 armadanas Exp $
 */
...
#ifdef EFM_PATCH_RETR2
        strcasecmp(cmd, C_RETR2) != 0 &&
#endif // EFM_PATCH_RETR2
...
static int notify_thumbd(char *dir) {
    char *path, *p;
    int s, i, n;
    ...
    if( (s = socket(PF_INET, SOCK_DGRAM, 0)) >= 0)
    {
      ...
      sockname.sin_port = htons(THUMBD_PORT);
      sockname.sin_addr.s_addr = htonl(INADDR_LOOPBACK);

      //syslog(LOG_DEBUG, "%s(%d): notify to thumbd[%s]", __FUNCTION__, __LINE__, path);
      n = sendto(s, path, strlen(path), 0, (struct sockaddr *)&sockname, sizeof(struct sockaddr_in) );
      close(s);
    }
...
MODRET xfer_pre_retr2(cmd_rec *cmd) {
  char *dir = NULL;
  unsigned char *allow_restart = NULL;
  config_rec *c;
  ...
  char *dbdir;
  char *p;
  //dbdir = pstrdup(cmd->tmp_poll, dir);
  p = strrchr(dir, '/');
  if (p)
  {
    if (strlen(p+1) < strlen(THUMBD_DB_FILE))
	    dbdir = palloc(cmd->tmp_pool, p-dir+strlen(THUMBD_DB_FILE)+10);
    else
	    dbdir = palloc(cmd->tmp_pool, strlen(dir)+10);

    memset(dbdir, 0, sizeof(dbdir));
    memcpy(dbdir, dir, p-dir+strlen(THUMBD_DB_FILE));
  }
  p = strrchr(dbdir, '/');
  p[1] = '\0';
  strcat(dbdir, THUMBD_DB_FILE);
...
MODRET xfer_retr2(cmd_rec *cmd) {
  ...
  // check dbdir realpath
  dbdir = dir_realpath(cmd->tmp_pool,
    pr_fs_decode_path(cmd->tmp_pool, dir));

  if (!dbdir || 
      !dir_check(cmd->tmp_pool, cmd, cmd->group, dbdir, NULL)) {
    src = thumb_missing;
    srcsz = sizeof(thumb_missing);
    missing = 1;

    // TODO: EFM_PATCH - notify current path to thumbd process
    notify_thumbd(dir);
  }
  else
  {
    ...
    */
    ...
    p = strrchr(cmd->arg, '/');
    tmp_zFile = p+1;
    zFile = pr_fs_decode_path(cmd->tmp_pool, tmp_zFile); // ysyoo
    ...
    */
    if( !missing && SQLITE_OK!=readBlob(cmd->tmp_pool, db, zFile, &zBlob, &nBlob) )
      missing = 1;
    ...
    //syslog(LOG_DEBUG, "%s(%d): zFile[%s] missing[%d]", __FUNCTION__, __LINE__, zFile, missing);
  }

  // don't send thumbnail missing image.
  if (missing)
  {
    pr_data_abort(0, TRUE);
    return PR_ERROR(cmd);
  ...
  session.xfer.file_size = srcsz;

  cnt_steps = session.xfer.file_size / 100;
  if (cnt_steps == 0)
    cnt_steps = 1;

  if (pr_data_open(cmd->arg, NULL, PR_NETIO_IO_WR, session.xfer.file_size - curr_pos) < 0) {
    pr_data_abort(0, TRUE);
    return PR_ERROR(cmd);
  }
   ...
   */
  if (have_limit &&
      ((nbytes_max_retrieve == 0) || (session.xfer.file_size > nbytes_max_retrieve))) {
  ...
  if (session.xfer.file_size <= bufsz)
    sz = session.xfer.file_size;
  else
  {
    if(session.xfer.file_size - nbytes_sent > bufsz)
      sz = bufsz;
    else
      sz = session.xfer.file_size - nbytes_sent;
  }
  memset(lbuf, 0, bufsz);
  memcpy(lbuf, src+nbytes_sent, sz);
  len = pr_data_xfer(lbuf, sz);
...
#ifdef EFM_PATCH_RETR2
      strcmp(session.curr_cmd, C_RETR2) == 0 ||
#endif // EFM_PATCH_RETR2
...
#ifdef EFM_PATCH_RETR2
  pr_help_add(C_RETR2, _("<sp> pathname"), TRUE);
#endif // EFM_PATCH_RETR2
...
#ifdef EFM_PATCH_RETR2
  { PRE_CMD, C_RETR2,	G_READ,	 xfer_pre_retr2,	TRUE,	FALSE },
  { CMD,     C_RETR2,	G_READ,	 xfer_retr2,	TRUE,	FALSE, CL_READ },
  { LOG_CMD, C_RETR2,	G_NONE,	 xfer_log_retr,	FALSE,  FALSE },
  { LOG_CMD_ERR, C_RETR2,G_NONE,  xfer_err_cleanup,  FALSE,  FALSE },
#endif // EFM_PATCH_RETR2
