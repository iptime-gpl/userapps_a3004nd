...
/* Core FTPD module
 * $Id: mod_core.c,v 1.2 2014/04/18 07:07:28 thki81 Exp $
 */
...
//debug_Aaron on 09/13/2007
extern int TransferLogSize;
...
//debug_Aaron on 09/13/2007
MODRET set_transferlogsize(cmd_rec *cmd) {
  int count;
  char *endp;
  ...
  if (!strcasecmp(cmd->argv[1],"none"))
    ...
    count = (int)strtol(cmd->argv[1],&endp,10);

    if ((endp && *endp) || count < 1)
      CONF_ERROR(cmd, "argument must be 'none' or a number greater than 0");
  }

  TransferLogSize = count;
  return HANDLED(cmd);
}
...
MODRET set_recvfile(cmd_rec *cmd) {  // Zachary
  int bool = -1;
  config_rec *c = NULL;
...
MODRET core_rmd(cmd_rec *cmd) {
...
#ifdef USE_EFMLOG
  {
	...
	sprintf(log_user, "%s(%s)",session.user, pr_netaddr_get_ipstr(session.c->remote_addr));
        efmlog( efmlog_locale,
                LOG_EFM, EFMLOG_FTP_RMDIR,
                log_user,
                getpid(),
                (char *)(EFMLOG_TEXT(efmlog_locale, "FTP_RMDIR")), dir);
  }
#endif
  return PR_HANDLED(cmd);
}
...
MODRET core_mkd(cmd_rec *cmd) {
...
#ifdef USE_EFMLOG
  {
	...
	sprintf(log_user, "%s(%s)",session.user, pr_netaddr_get_ipstr(session.c->remote_addr));
        efmlog( efmlog_locale,
                LOG_EFM, EFMLOG_FTP_MKDIR,
                log_user,
                getpid(),
                (char *)(EFMLOG_TEXT(efmlog_locale, "FTP_MKDIR")), dir);
  }
#endif
...
MODRET core_dele(cmd_rec *cmd) {
  ...
  {
        char log_user[64];
	char *cwd =get_alloc_cwd();
	char *filename = pr_fs_decode_path(cmd->tmp_pool, cmd->arg);
	sprintf(log_user, "%s(%s)",session.user, pr_netaddr_get_ipstr(session.c->remote_addr));
        efmlog( efmlog_locale,
                LOG_EFM, EFMLOG_FTP_FILE_DELETE,
                log_user,
                getpid(),
                (char *)(EFMLOG_TEXT(efmlog_locale, "FTP_FILE_DELETE")), cwd, filename);
	if(cwd) free(cwd);
  }
#endif
  return PR_HANDLED(cmd);
...
static conftable core_conftab[] = {
  ...
  /* Deprecated */
  { "Bind",			set_bind,			NULL },
  { "receivefile",              set_recvfile,                   NULL },  // Zachary

//debug_Aaron on 09/13/2007 limit the Transfer
  { "TransferLogSize",         set_transferlogsize,          NULL },  
