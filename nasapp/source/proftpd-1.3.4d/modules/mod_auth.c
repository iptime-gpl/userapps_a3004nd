 ...
 * $Id: mod_auth.c,v 1.1.1.1 2014/04/11 02:05:20 thki81 Exp $
 */
...
#ifdef USE_EFMLOG
  // Login block check.
  if (check_tally_auth(TALLY_SERVICE_FTP, TALLY_AUTH_CHECK, (char *)pr_netaddr_get_ipstr(session.c->remote_addr)))
  {
	  efmlog_simple( EFMLOG_FTP_LOGIN_FAIL, SYSTEM_ACCOUNT, "FTP_LOGIN_BLOCKED", (char*)pr_netaddr_get_ipstr(session.c->remote_addr));
	  return -1;
  }
#endif // USE_EFMLOG
  config_rec *c = NULL;
...
#ifdef USE_EFMLOG
  {
	sprintf(log_user, "root(%s)",pr_netaddr_get_ipstr(session.c->remote_addr)),
	efmlog( efmlog_locale, 
		LOG_EFM, EFMLOG_FTP_USER_ROOT_DENY, 
		log_user,
		getpid(), 
		(char *)(EFMLOG_TEXT(efmlog_locale, "FTP_USER_ROOT_DENY")));
  }
#endif
      return 0;
    }
...
#ifdef USE_EFMLOG
  {
	sprintf(log_user, "ANON(%s)",pr_netaddr_get_ipstr(session.c->remote_addr)),
	efmlog( efmlog_locale, 
		LOG_EFM, EFMLOG_FTP_USER_ANON_DENY, 
		log_user,
		getpid(), 
		(char *)(EFMLOG_TEXT(efmlog_locale, "FTP_USER_ANON_DENY")));
  }
#endif
      goto auth_failure;
    }
...
# endif /* __hpux */
  ...
  {
	  /* for hdd sleep mode */
	  //set_hdd_wakeup_flag(1);
	  wait_for_hdd_wakeup("FTP");

	sprintf(log_user, "%s(%s)",origuser, pr_netaddr_get_ipstr(session.c->remote_addr)),
	efmlog( efmlog_locale, 
		LOG_EFM, EFMLOG_FTP_USER_LOGIN_SUCCESS, 
		//(c != NULL) ? "ANON" : log_user,
		log_user,
		getpid(), 
		(char *)(EFMLOG_TEXT(efmlog_locale, "FTP_USER_LOGIN_SUCCESS")));

		check_tally_auth(TALLY_SERVICE_FTP, TALLY_AUTH_SUCCESS, (char *)pr_netaddr_get_ipstr(session.c->remote_addr)); // success
  }
#endif
...
#ifdef USE_EFMLOG		
	check_tally_auth(TALLY_SERVICE_FTP, TALLY_AUTH_FAIL, (char *)pr_netaddr_get_ipstr(session.c->remote_addr)); // fail
#endif // USE_EFMLOG
      pr_session_end(0);
    }
...
MODRET auth_pass(cmd_rec *cmd) {
  ...
  {
	char log_user[64];
	sprintf(log_user, "%s(%s)",user, pr_netaddr_get_ipstr(session.c->remote_addr)),
	efmlog( efmlog_locale, 
		LOG_EFM, EFMLOG_FTP_USER_PASSWORD_WRONG, 
		log_user,
		getpid(), 
		(char *)(EFMLOG_TEXT(efmlog_locale, "FTP_USER_PASSWORD_WRONG")));
	check_tally_auth(TALLY_SERVICE_FTP, TALLY_AUTH_FAIL, (char *)pr_netaddr_get_ipstr(session.c->remote_addr)); // fail
  }
#endif
