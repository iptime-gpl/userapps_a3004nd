 ...
 *
 * $Id: session.c,v 1.1.1.1 2014/04/11 02:05:18 thki81 Exp $
 */
...
void pr_session_end(int flags) {
  int exitcode = 0;
    ...
    {
        char log_user[64];
        sprintf(log_user, "%s(%s)",session.user, pr_netaddr_get_ipstr(pr_netaddr_get_sess_remote_addr()));
        efmlog( efmlog_locale,
                LOG_EFM, EFMLOG_FTP_USER_LOGOUT,
                log_user,
                getpid(),
                (char *)(EFMLOG_TEXT(efmlog_locale, "FTP_USER_LOGOUT")));
    }
#endif
