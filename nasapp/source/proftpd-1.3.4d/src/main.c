 ...
 * $Id: main.c,v 1.3 2015/07/06 04:55:50 thki81 Exp $
 */
  ...
  if (c == NULL ||
    ...
    if (c &&
      ...
      pr_response_send(R_220, "%s", server_ident);
               ...
               *defer_welcome == TRUE) {
#ifdef USE_IPTIME_WELCOME
      pr_response_send(R_220, "ipTIME_FTPD " PROFTPD_VERSION_TEXT
        " Server ready.");
#else
      ...
      if (c)
      {
	      ...
	      {
		      if (!strcasecmp(c->argv[1], "EUC-KR"))
			      utf8_charset = 0;
	      }
	      pr_response_send(R_220, "ipTIME_FTPD " PROFTPD_VERSION_TEXT
		" Server (%s) [%s] %s", server->ServerName, serveraddress, utf8_charset?"UTF-8":"EUC-KR");
      }
      else
      {
	      pr_response_send(R_220, "ipTIME_FTPD " PROFTPD_VERSION_TEXT
