 ...
 * $Id: data.c,v 1.3 2016/01/05 01:03:30 thki81 Exp $
 */
...
#ifdef USE_EFMLOG
  if (strcmp(reason, "file list") && strcmp(reason, "MLSD")) {
        char log_user[64];
	char *cwd;
	char *filename = pr_fs_decode_path(session.d->pool, reason);

        sprintf(log_user, "%s(%s)",session.user, pr_netaddr_get_ipstr(session.d->remote_addr));

	cwd=get_alloc_cwd();
	if(strcmp(session.curr_cmd, "STOR"))
	{
        	efmlog( efmlog_locale,
                        LOG_EFM, EFMLOG_FTP_FILE_GET,
                        log_user, getpid(),
                        (char *)(EFMLOG_TEXT(efmlog_locale, "FTP_FILE_GET")), 
	 	       cwd,
	 	       filename);
	}
	else
	{
		efmlog( efmlog_locale,
                        LOG_EFM, EFMLOG_FTP_FILE_PUT,
                        log_user, getpid(),
                        (char *)(EFMLOG_TEXT(efmlog_locale, "FTP_FILE_PUT")), 
	 	       filename,
	 	       cwd);
	}
  ...
  if (res == 0 &&
    ...
    if (res < 0) {
      ...
      if (pr_cmd_cmp(cmd, PR_CMD_APPE_ID) == 0 ||
          ...
          pr_cmd_cmp(cmd, PR_CMD_RETR2_ID) == 0 ||
