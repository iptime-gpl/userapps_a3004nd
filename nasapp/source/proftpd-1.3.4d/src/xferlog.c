 ...
 * $Id: xferlog.c,v 1.1.1.1 2014/04/11 02:05:18 thki81 Exp $
 */
...
//debug_Aaron on 09/13/2007 limit the log file size
int TransferLogSize = 20;
...
//debug_Aaron on 09/14/2007 limit the log file size
unsigned long offset;
struct stat sbuf;
if (fstat(xferlogfd, &sbuf) != 0)
	return 0;

//sprintf(buf, "Current file size=%ld\n", sbuf.st_size);
//write(xferlogfd, buf, strlen(buf));
if (sbuf.st_size > TransferLogSize * 1024)  /* because GUI only allocate 64KB for display */
{
//	sprintf(buf, "Rewind log file..., TransferLogSize=%d, file size=%ld\n", TransferLogSize, sbuf.st_size);
//        write(xferlogfd, buf, strlen(buf));
	//system("echo \"\" > /system/xferlog");  
	return 0;
}
