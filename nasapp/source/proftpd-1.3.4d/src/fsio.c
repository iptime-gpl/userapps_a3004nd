 ...
 * $Id: fsio.c,v 1.1.1.1 2014/04/11 02:05:18 thki81 Exp $
 */
...
static int sys_mkdir(pr_fs_t *fs, const char *path, mode_t mode) {
	...
	if (pdata)
	{
		...
		strcpy(pdata, path);
		ptr = strrchr(pdata, '/');
		eptr = pdata + strlen(pdata) - 1;
		/* check & trim spaces of folder name */
		if (ptr)
		{
		       	*ptr = '\0';
			ptr++;
			...
			if (eptr && eptr++) *eptr = '\0';
			sprintf(pdata, "%s/%s", pdata, ptr);
		}

		ret = mkdir(pdata, mode);
		free(pdata);
	}
