...
static int gred_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	opt.Scell_log = wlog;
/*
add append flag since we maintain one large setup and we
want have it there all the time (not recreate it when 
we are changing a single paramater)
	n->nlmsg_flags|=NLM_F_APPEND;
	n->nlmsg_flags|=NLM_F_REPLACE;
*/
	n->nlmsg_flags|=NLM_F_APPEND;
