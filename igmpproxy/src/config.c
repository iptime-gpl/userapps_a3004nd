    ...
    while(token != NULL) {
        if(strcmp("altnet", token)==0) {
            ...
            *anetPtr = parseSubnetAddress(token);
            ...
            tmpPtr->name = (char *)malloc( sizeof(char) * strlen("ethx.x") );
            if(tmpPtr->name == NULL) log(LOG_ERR, 0, "Out of memory.");
            get_wan_ifname(WAN1_NAME, tmpPtr->name);
            tmpPtr->state = IF_STATE_UPSTREAM;
	    log(LOG_NOTICE, 0, "upstream : %s ", tmpPtr->name);
        }
        else if(strcmp("downstream", token)==0) {
            // Downstream
            IF_DEBUG log(LOG_DEBUG, 0, "Config: IF: Got downstream token.");
            tmpPtr->name = (char *)malloc( sizeof(char) * strlen(IF_LOCAL) );
            if(tmpPtr->name == NULL) log(LOG_ERR, 0, "Out of memory.");
            strcpy(tmpPtr->name, IF_LOCAL);
            tmpPtr->state = IF_STATE_DOWNSTREAM;
	    log(LOG_NOTICE, 0, "downstream : %s ", tmpPtr->name);
        }
