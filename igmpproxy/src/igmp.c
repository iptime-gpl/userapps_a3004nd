...
extern void update_router_port(uint32 src);
/*
...
#define UPNP_MCAST_ADDR htonl(0xEFFFFFFA)   // 239.255.255.250
...
void acceptIgmp(int recvlen) {
    ...
    char local_ip[20], local_mask[20];
    ...
    if(dst == UPNP_MCAST_ADDR) 
    {
            IF_DEBUG log(LOG_DEBUG, 0, "****     UPNP mcast addr \n");
	    return;
    }
	...
	return 0;
	    ...
	    get_ifconfig(IF_LOCAL, local_ip, local_mask);
	    if (check_same_network(local_ip, inetFmt(src, s1), local_mask))
	    {
            	log(LOG_WARNING, 0, "local origin.");
		return;
	    }
	...
	// Detect a Router port
	get_ifconfig(IF_LOCAL, local_ip, local_mask);
	if (check_same_network(local_ip, inetFmt(src, s1), local_mask))
	{
		IF_DEBUG log(LOG_DEBUG, 0,"=======> IGMP Query (s:%08x, g:%08x)", ntohl(src), ntohl(group));
		update_router_port(src);
	}
