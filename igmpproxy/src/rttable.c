...
void ipmrcache_gc(void)
{
	...
	while (fgets(buffer, 128, fp))
	{
		...
		if(croute.group == UPNP_MCAST_ADDR )
			continue;
		...
		get_wan_ifname(WAN1_NAME, wanif);
		Dp = getIfByName(wanif);
		if (Dp)
		{
			croute.vifBits = Dp->index; 
			internUpdateKernelRoute(&croute, 0);
			log(LOG_DEBUG, 0, "vifBits = [%x]\n", croute.vifBits);
		}
		sendJoinLeaveUpstream(&croute, 0);
		//syslog_msg(0, "ipmrcache_gc() - group %08x orgin %08x\n", croute.group,croute.originAddr);
	}
