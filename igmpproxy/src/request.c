...
void acceptLeaveMessage(uint32 src, uint32 group) {
	...
	// SWITCH SUPPORT
	remove_member(ntohl(group), ntohl(src), 1);
...
void sendGroupSpecificMemberQuery(void *argument) {
    ...
    if (!get_wan_link(WAN1_NAME)) return;

    if (!igmp_proxy_running) return;	

    // Cast argument to correct type...
    gvDesc = (GroupVifDesc*) argument;
...
extern void query_specific_members(void);
...
void sendGeneralMembershipQuery() {
    ...
    timer_setTimer(conf->queryResponseInterval * 5, ageActiveRoutes, NULL);
    ...
    if(conf->startupQueryCount>0) {
        ...
        timer_setTimer(INTERVAL_QUERY_RESPONSE+10, query_specific_members, NULL);
    }


}
 		
void sendSpecificMembershipQueryToSpecificMember(uint32 dst, uint32 group) {
    struct  IfDesc  *Dp;
    int             Ix;
	...
	{
            if ((Dp->state == IF_STATE_DOWNSTREAM) &&
	        (Dp->allowednets && Dp->allowednets->subnet_mask && Dp->allowednets->subnet_addr) &&
	        ((dst & Dp->allowednets->subnet_mask) == Dp->allowednets->subnet_addr) )
	    {
                // Send the membership query...
                sendIgmp(Dp->InAdr.s_addr, dst, 
                         IGMP_MEMBERSHIP_QUERY, INTERVAL_UNICAST_QUERY_RESPONSE * IGMP_TIMER_SCALE, group, 0);
                
                IF_DEBUG log(LOG_DEBUG, 0, "Sent specific membership query from %s to %s.",
                    inetFmt(Dp->InAdr.s_addr,s1), inetFmt(group,s2));
            }
        }
    }
}
