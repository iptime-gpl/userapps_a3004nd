...
static void check_igmp_proxy_or_snoop(void)
{
	char wanip[20];
	igmp_proxy_running = (get_wan_link(WAN1_NAME) && get_wan_ip(WAN1_NAME, wanip));
}

/**
*   Program ain method. Is invoked when the program is started
...
int igmpProxyInit() {
     ...
     */
    {
		...
		if (strcmp(inet_ntoa(Dp->InAdr), "192.168.255.1") && 
		    strcmp(inet_ntoa(Dp->InAdr), "192.168.255.250"))
		{
                    IF_DEBUG log(LOG_DEBUG, 0, "add vif : %s ", inet_ntoa(Dp->InAdr));
                    addVIF( Dp );
                    vifcount++;
		}
            }
...
void igmpProxyCleanUp() {
    ...
    remove_all_groups();    // delete all mac tables
