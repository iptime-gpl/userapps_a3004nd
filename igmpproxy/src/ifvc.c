...
void buildIfVc() {
     ...
     */
    {
        ...
        for ( IfPt = IfVc; IfPt < IfEp; IfPt++ ) {
            struct ifreq IfReq;
            char FmtBu[ 32 ], ip_addr[32], netmask[32];
	...
	{ // EFM, ysyoo, 2015-10-06
		char local_ip[32],sm[32], wanif[32];

		get_wan_ifname(WAN1_NAME, wanif);
		get_ifconfig(IF_LOCAL,local_ip,sm);

		if (strcmp(wanif, IfDescEp->Name) && strcmp(inet_ntoa(IfDescEp->InAdr), local_ip))
		{
			IfDescEp++;
			continue;
		}
