...
void switch_init()
{
	...
	// IGMP snooping Control Reg
	value = rareg(READ, 0x10110018, 0);
	value = value | (0x1 << 18); //Enable Router Port Learning
	rareg(WRITE, 0x10110018, value);

	/* to check default IGMP flooding rule*/
	/* IGMP report forward to cpu/query: default policy*/
	/* IGMP Message Control Reg */
	value = rareg(READ, 0x1011001c, 0);
	value = value | (0x5 << 12); //  Forward system default and CPU port
	rareg(WRITE, 0x1011001c, value);
