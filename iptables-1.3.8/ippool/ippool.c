...
#define IP_POOL_T_BITMAP	BITMAP_TYPE
#define IP_POOL_T_ALLIP		ALLIP_TYPE
	...
	while (SCAN_EOF != (res=get_index_line(fp, &poolname, &type, &arg))) {
		if (poolname && 0 == strcmp(poolname, name)) {
			if (type) {
				...
				else if( !strcmp(type, "allip"))
					conf_type = IP_POOL_T_ALLIP;
...
static char *get_name(ip_pool_t idx)
{
	FILE *fp;
...
static void do_list(ip_pool_t pool)
{
	...
	if (0 == getsockopt(fd, SOL_IP, SO_IP_POOL, &req, &reqlen)) {
		...
		printf(" %s", (req.type == IP_POOL_T_BITMAP)?"bitmap":"allip");
