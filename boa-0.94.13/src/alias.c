...
/* $Id: alias.c,v 1.7 2014/04/16 04:08:23 ysrtl98 Exp $ */
...
int translate_uri(request * req)
{
...
#else
    strcpy(srv_ip, "192.168.255.254");
#endif
    if(!strcmp(req->local_ip_addr, srv_ip) && !strcmp(req->request_uri, "/cgi-bin/timepro.cgi"))
    {
        strcpy(req->request_uri, "/dummy");
    }
    // ysyoo, URL_REDIRECT
    else if(get_url_redirect_option(buffer) &&
       !strcmp(req->local_ip_addr, srv_ip) && !strcmp(req->request_uri, "/"))
    {
        strcpy(req->request_uri, "/dummy");
    }
#endif
#if defined(USE_IPTIME_SERVICE_NETWORK) && defined(USE_NETDETECT)
    sprintf(srv_ip, "%s.250", srv_net);
    if(!strcmp(req->local_ip_addr, srv_ip) && netdetect_get_on_off())
    {
        strcpy(req->request_uri, "/dummy");
    }
#endif
    ...
    if (user_dir && req->request_uri[1] == '~') {
        ...
        if (virtualhost) {
#ifdef USE_READONLY_WEPHOME
            char fake_local_ip[32];
#endif
            buffer[l1] = '/';
#ifdef USE_READONLY_WEPHOME
            strcpy(fake_local_ip, "192.168.0.1");

            l3 = strlen(fake_local_ip);
            memcpy(buffer + l1 + 1, fake_local_ip, l3);
...
int init_script_alias(request * req, alias * current1, int uri_len)
{
    ...
    /* we have path_info if c == '/'... still have to check for query */
        ...
        req->path_info = strdup(pathname + idx);
        if (!req->path_info) {
            send_r_error(req);
            WARN("unable to strdup pathname + idx for req->path_info");
            return 0;
        }
        pathname[idx] = '\0'; /* strip path_info from path */
        path_len = strlen(req->path_info);
        /* we need to fix script_name here */
        /* idx points into pathname, which is
         * realname/cginame/foo
         * and idx points to the '/foo' part
         */
        ...
        if (!req->path_translated && user_dir && req->path_info[1] == '~') {
            ...
            p = strchr(pathname + idx + 1, '/');
            if (p)
                *p = '\0';

            user_homedir = get_home_dir(pathname + idx + 2);
            if (p)
                *p = '/';
