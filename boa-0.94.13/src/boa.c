...
#ifdef USE_HTTPD_RECOVERY
#ifndef USE_NEW_LIB
    httpd_start_callback();
#endif
#endif
...
#ifdef USE_HTTP_SESSION
char password_cache[128];
char id_cache[128];
static void init_id_pass(void)
{
	get_id_password(id_cache,password_cache);
}
...
int check_password_cache(char *id, char *passwd)
{
        ...
        if(!strcmp(password_cache,"") && !strcmp(passwd,""))
                return 0;
        if(!strcmp(passwd,password_cache))
                return 0;
        return 1;
}

int exist_password = 0;
int passwd_check(void)
{
        if(!exist_password)
        {
                if(exist_passwd())
                        exist_password = PASSWORD_EXIST;
                else
                        exist_password = PASSWORD_NOT_EXIST;
        }
...
void create_httpd_pid(void)
{
	...
	FILE *pidfile;
        ...
        if((pidfile = fopen( "/var/run/httpd.pid", "w+"))!=NULL)
        {
