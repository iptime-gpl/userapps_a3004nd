...
/* $Id: request.c,v 1.13 2016/11/15 06:39:13 sgbcmac Exp $*/
...
#ifdef USE_CSRF_BLOCK
    if (req->header_referer && !check_referer(req->header_referer))
    {
...
#ifdef USE_URL_REDIRECT
	   if(!get_url_redirect_option(buf))
#endif
	   { 	
               char buf_referer[256];

               strncpy(buf_referer, req->header_referer, 255);
       	       syslog_msg(SYSMSG_LOG_WANRING, "%s%s", SYSLOG_MSG_CSRF_BLOCK, buf_referer);
	   }
...
#ifdef USE_AUTH
#ifdef USE_HTTP_SESSION
    if((passwd_check()==PASSWORD_EXIST) && !auth_authorize(req))
	    return 0;
#else
    if(exist_passwd() && !auth_authorize(req))
	    return 0;
#endif
#endif
