...
/* $Id: read.c,v 1.21 2016/10/10 03:38:56 sgbcmac Exp $*/

#include "boa.h"

#ifdef USE_RDC_R3210
#define MAX_BOA_FIRMWARE_SIZE 0x2001f4 //2097652
#else
#ifdef USE_NEW_LIB
#define MAX_BOA_FIRMWARE_SIZE hwinfo_get_max_firmware_size()
#else
#define MAX_BOA_FIRMWARE_SIZE MAX_FIRMWARE_SIZE
#endif
#endif

#ifdef USE_NEW_UPGRADE_METHOD
#include "linosconfig.h"
static int check_header( char *head, int size );
static int check_last( char *head, int size );
//static char *memsearch2( char *input, char *key, int size);
...
int check_valid_session(request *req)
{
	char session_id[HTTP_SESSION_ID_LEN+16];
	char method[32];
	int i,foundit=0;
	char *cookie;

	strcpy(method,"");
	get_http_auth_method(method);
	if(strcmp(method,"session")) /* for basic method, always OK */
		return 1;
	for(i=0;i<CGI_ENV_MAX && req->cgi_env[i]; i++)
	{
		if(!strncmp(req->cgi_env[i],"HTTP_COOKIE",strlen("HTTP_COOKIE")))
		{
			foundit=1;
			break;
		}
	...
	//syslog_msg(1,"cgi_env -> %s", req->cgi_env[i]);
	cookie=req->cgi_env[i];
	if(!cookie) return 0;

	cookie+=strlen("HTTP_COOKIE")+1; /* 1 -> '=' */
	//syslog_msg(1,"cookie --> %s", cookie);
	
	/* session method but no session id in cookie  */
	if(!extract_session_id_from_cookie(cookie,session_id)) 
		return 0;
	//syslog_msg(1,"session_id : %s %s", session_id, req->remote_ip_addr);
	if(!check_session("httpcon",session_id,req->remote_ip_addr))
	{
		//syslog_msg(1,"Check Session is NULL");
		return 0;
	}
	return 1;
}
...
int write_body(request * req)
{
...
#ifdef USE_NEW_UPGRADE_METHOD
    /**** upgrade special call *****/ 
    if(req->filepos == 0)
    {
	    /* parsing header whether it is upgrade image */
	    if(check_header(req->header_line, req->filesize))
	    {
#ifdef USE_HTTP_SESSION
		if(check_valid_session(req))
		{
#endif
    		upgrade_mode();
		/* file open */
		upgrade_start = 1;
		unlink("/tmp/firmware");
		system("rm -rf /tmp/lost+found");
		set_sw_upgrade_status(UPGRADE_STARTED);
#ifdef USE_HTTP_SESSION
		}
#endif
	    }
	...
	tmp_fp = fopen("/tmp/firmware", "a");
    	bytes_written = fwrite( req->header_line, 1 , bytes_to_write, tmp_fp );
	fclose(tmp_fp);

    	if( req->filepos && (check_last( req->header_line, bytes_written ) || ((req->filepos + bytes_written) > MAX_BOA_FIRMWARE_SIZE))) 
	{
		set_sw_upgrade_status(UPGRADE_FILE_WRITE_END);
		upgrade_start = 0;
		fprintf(stderr, "upload complete..");
#ifdef USE_UPGRADE_SIGNAL
		kill(1,SIGINT); /* upgrade signal */ 
#endif
...
static int check_header( char *head, int size )
{
	...
	get_line(head, deli);
#if 0
	fprintf(stderr, "deli : %s\n", deli );
#endif

	sptr = strstr( head, "form-data" );
	if(sptr)
	{
		sptr = strstr(sptr, "name=");
		if(sptr)
		{
			sptr += strlen("name=") + 1;
			if(!strncmp( sptr, "upgrade", 7 ))
			{
				/* check small file */
				if(strstr( head + strlen(deli), deli ) || size <= 8192)
				{
					set_sw_upgrade_status(UPGRADE_SMALL_FILE);
					return 0;
				}
...
static char *memsearch2( char *input, char *key, int size)
{
        ...
        for( iptr=input, i = 0 ; i < size; i++ )
        {
                for( kptr=key, j = 0 ; j < key_size; j++)
                {
                        if( kptr[j] != iptr[i+j])
                                break;
                }
...
#ifndef USE_UPGRADE_SIGNAL
		system("/upgrade-bin/updaemon &");
#endif
#ifdef USE_NO_REBOOT_FOR_INVALID_FIRMWARE
		if(si_dram_size_mb()<32)
#endif
			set_upgrade_mode(1);
	}
	else
		fprintf(stderr,"second request..\n");
