...
/* $Id: cgi.c,v 1.8 2015/05/14 06:51:37 mt7621 Exp $ */
...
void create_common_env()
{
     ...
     */
    common_cgi_env[idx++] = env_gen_extra("PATH",
                                            ((cgi_path != NULL) ? cgi_path : DEFAULT_PATH), 0);
    common_cgi_env[idx++] = env_gen_extra("SERVER_SOFTWARE", SERVER_VERSION, 0);
    common_cgi_env[idx++] = env_gen_extra("SERVER_NAME", server_name, 0);
    common_cgi_env[idx++] = env_gen_extra("GATEWAY_INTERFACE", CGI_VERSION, 0);

    common_cgi_env[idx++] =
        env_gen_extra("SERVER_PORT", simple_itoa(server_port), 0);

    /* NCSA and APACHE added -- not in CGI spec */
    /* common_cgi_env[idx++] = env_gen_extra("DOCUMENT_ROOT", document_root); */

    /* NCSA added */
    /* common_cgi_env[idx++] = env_gen_extra("SERVER_ROOT", server_root); */

    /* APACHE added */
    common_cgi_env[idx++] = env_gen_extra("SERVER_ADMIN", server_admin, 0);
    common_cgi_env[idx] = NULL;

    /* Sanity checking -- make *sure* the memory got allocated */
    if (idx > COMMON_CGI_COUNT) {
        log_error_time();
...
int complete_env(request * req)
{
    ...
    if(req->http_version) my_add_cgi_env(req, "SERVER_PROTOCOL", req->http_version);
    ...
    if(req->script_name) my_add_cgi_env(req, "SCRIPT_NAME", req->script_name);
    ...
    if(req->header_user_agent) my_add_cgi_env(req, "USER_AGENT", req->header_user_agent);

#ifdef USE_COOKIE
    if(req->cookie) my_add_cgi_env(req, "HTTP_COOKIE", req->cookie);
#endif
    ...
    if(req->authorization)
    {
	    ...
	    base64decode2(user_pass,req->authorization+6,0x100);
	    ptr=strchr(user_pass,':');
	    if(ptr)
	    {
		    *ptr=0;
		    //fprintf(stderr,"USER_ID => %s\n", user_pass);
	    	    my_add_cgi_env(req, "USER_ID", user_pass);
	    }
    }
#endif
...
#ifdef USE_RT6856
/* In RT6856/MT7620/MT7621 SDK, fork sometimes fail, so web page can't be printed.
 * this workaround reduce the fork fail : scchoi 2015-05-14 */ 
if(child_pid == -1)
{
        perror("fork-err1");
	//syslog_msg(1,"Try one more fork\n");
    	child_pid = fork();
}
#endif
