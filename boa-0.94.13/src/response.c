...
void send_r_not_found(request * req)
{
...
#if defined USE_SYSINFO && defined USE_WL_IPTIME_HELPER
	{
		...
		if(sw.wl_helper.redirect && (istatus_get_intvalue_direct("iptime_wlconfig_start") == 1))
		{
			char url2[256];
		...
		if(sw.wl_helper2.redirect && (istatus_get_intvalue_direct("iptime_wlconfig_start") == 1))
		{
			char url2[256];
