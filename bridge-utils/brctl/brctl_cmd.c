...
void br_cmd_setportmap(struct bridge *br, char *arg0, char *arg1)
{
	int map;
	struct port *p;
	...
	sscanf(arg1, "%x", &map);
	br_set_port_map(p, map);
}
#endif

#ifdef USE_TRUNK_LAN_PORTS_ARE_BRIDGE_PORTS
void br_cmd_settrunkmap(struct bridge *br, char *arg0, char *arg1)
{
	int map;
	struct port *p;
...
#elif defined(USE_TRUNK_LAN_PORTS_ARE_BRIDGE_PORTS)
	printf("bridge name\tbridge id\t\tSTP enabled\tinterfaces\ttrunkmap\n");
#else
...
void __dump_fdb_entry(struct bridge *br, struct fdb_entry *f)
{
...
#endif
#ifdef USE_PORT_BASED_VLAN_IN_BRIDGE
	{1, 2, "setportmap", br_cmd_setportmap},
#endif
#ifdef USE_TRUNK_LAN_PORTS_ARE_BRIDGE_PORTS
	{1, 2, "settrunkmap", br_cmd_settrunkmap},
#endif
