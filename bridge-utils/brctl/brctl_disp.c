...
void br_dump_interface_list(struct bridge *br)
{
	...
	if (p != NULL) {
		...
		printf("%s\t\t%08x", if_indextoname(p->ifindex, ifname), p->info.port_map);
#elif defined(USE_TRUNK_LAN_PORTS_ARE_BRIDGE_PORTS)
		printf("%s\t\t%08x", if_indextoname(p->ifindex, ifname), p->info.trunk_map);
#else
	...
	while (p != NULL) {
#ifdef USE_PORT_BASED_VLAN_IN_BRIDGE
		printf("\t\t\t\t\t\t\t%s\t\t%08x\n", if_indextoname(p->ifindex, ifname), p->info.port_map);
#elif defined(USE_TRUNK_LAN_PORTS_ARE_BRIDGE_PORTS)
		printf("\t\t\t\t\t\t\t%s\t\t%08x\n", if_indextoname(p->ifindex, ifname), p->info.trunk_map);
#else
