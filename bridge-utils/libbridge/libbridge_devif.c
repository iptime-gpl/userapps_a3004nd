...
int br_set_port_map(struct port *p, int port_map)
{
	if (br_device_ioctl(p->parent, BRCTL_SET_PORT_MAP, p->index,
			    port_map, 0) < 0)
		return errno;
...
int br_set_trunk_map(struct port *p, int port_map)
{
	if (br_device_ioctl(p->parent, BRCTL_SET_TRUNK_MAP, p->index,
			    port_map, 0) < 0)
		return errno;
