/* $Id: upnpevents.c,v 1.2 2010/09/16 00:50:25 rtlwire Exp $ */
/* MiniUPnP project
...
upnp_event_create_notify(struct subscriber * sub)
{
	...
	if(!obj) {
		syslog(LOG_ERR, "%s: calloc(): ", "upnp_event_create_notify");
		return;
	}
	obj->sub = sub;
	obj->state = ECreated;
	...
	if(obj->s<0) {
		syslog(LOG_ERR, "%s: socket(): ", "upnp_event_create_notify");
		goto error;
	}
	if((flags = fcntl(obj->s, F_GETFL, 0)) < 0) {
		syslog(LOG_ERR, "%s: fcntl(..F_GETFL..): ",
		       "upnp_event_create_notify");
		goto error;
	}
	if(fcntl(obj->s, F_SETFL, flags | O_NONBLOCK) < 0) {
		syslog(LOG_ERR, "%s: fcntl(..F_SETFL..): ",
		       "upnp_event_create_notify");
		goto error;
	}
	if(sub)
...
upnp_event_notify_connect(struct upnp_event_notify * obj)
{
	...
	if(connect(obj->s, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		if(errno != EINPROGRESS && errno != EWOULDBLOCK) {
			syslog(LOG_ERR, "%s: connect(): ", "upnp_event_notify_connect");
			obj->state = EError;
		}
	}
}
...
static void upnp_event_send(struct upnp_event_notify * obj)
{
	...
	if(i<0) {
		syslog(LOG_NOTICE, "%s: send(): ", "upnp_event_send");
		obj->state = EError;
		return;
	}
...
static void upnp_event_recv(struct upnp_event_notify * obj)
{
	...
	if(n<0) {
		syslog(LOG_ERR, "%s: recv(): ", "upnp_event_recv");
		obj->state = EError;
		return;
	}
