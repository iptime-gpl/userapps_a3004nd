/* $Id: upnphttp.c,v 1.2 2010/09/16 00:50:25 rtlwire Exp $ */
/* Project :  miniupnp
...
CloseSocket_upnphttp(struct upnphttp * h)
{
	if(close(h->socket) < 0)
	{
		syslog(LOG_ERR, "CloseSocket_upnphttp: close(%d): ", h->socket);
	}
	h->socket = -1;
	h->state = 100;
}
...
/*
	...
	if(n < 0)
	{
		syslog(LOG_ERR, "Send404: send(http): ");
	}*/
...
/*
	...
	if(n < 0)
	{
		syslog(LOG_ERR, "Send501: send(http): ");
	}
*/
...
static void
sendXMLdescfile(struct upnphttp * h, char *xmlfile)
{
	FILE *fp;
	char *desc;
	int len = 0, desclen = 2048;
	int buf;
	...
	else
	{
		...
		while ((buf = fgetc(fp)) != EOF)
		{
			...
			{
				desclen += 256;
				desc = (char *)realloc(desc, desclen);
			}
			desc[len++] = (char)buf;
		}
		desc[len] = '\0';
