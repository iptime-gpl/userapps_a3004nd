/* $Id: options.c,v 1.3 2009/06/25 05:30:28 ysrt305x Exp $ */
/* MiniUPnP project
...
readoptionsfile(const char * fname)
{
	...
	while(fgets(buffer, sizeof(buffer), hfile))
	{
		...
		if(0 == memcmp(name, "allow", 5) || 0 == memcmp(name, "deny", 4))
		{
			...
			{
				fprintf(stderr, "1. parsing error file %s line %d : %s\n",
				        fname, linenum, name);
			}
			continue;
		}
		...
		{
			fprintf(stderr, "2. parsing error file %s line %d : %s\n",
			        fname, linenum, name);
			continue;
		}
		...
		*equals = '\0';
		...
		if(id == UPNP_INVALID)
		{
			fprintf(stderr, "3. parsing error file %s line %d : %s=%s\n",
