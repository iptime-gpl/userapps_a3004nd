/* $Id: daemonize.c,v 1.2 2010/09/16 00:50:25 rtlwire Exp $ */
/* MiniUPnP project
...
writepidfile(const char * fname, int pid)
{
	...
	{
		syslog(LOG_ERR, "Unable to open pidfile for writing %s: ", fname);
		return -1;
	}
	...
	else
	{
		if(write(pidfile, pidstring, pidstringlen) < 0)
			syslog(LOG_ERR, "Unable to write to pidfile %s: ", fname);
	}
