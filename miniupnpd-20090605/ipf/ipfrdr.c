/* $Id: ipfrdr.c,v 1.2 2010/09/16 00:50:25 rtlwire Exp $ */
/* MiniUPnP project
...
int init_redirect(void)
{
	...
	if(dev<0) {
		syslog(LOG_ERR, "open(\"%s\"): ", IPNAT_NAME);
		return -1;
	}
	dev_ipl = open(IPL_NAME, O_RDWR);
	if(dev_ipl < 0) {
		syslog(LOG_ERR, "open(\"%s\"): ", IPL_NAME);
		return -1;
	}
	return 0;
}
				   ...
				   const char * desc)
{
	...
	if (r == -1)
		syslog(LOG_ERR, "ioctl(SIOCADNAT): ");
	return r;
}
                  ...
                  u_int64_t * packets, u_int64_t * bytes)
{
	...
	do {
		if (ioctl(dev, SIOCGENITER, &obj) == -1) {
			syslog(LOG_ERR, "ioctl(dev, SIOCGENITER): ");
			goto error;
		}
...
delete_redirect_rule(const char * ifname, unsigned short eport, int proto)
{
	...
	do {
		if (ioctl(dev, SIOCGENITER, &obj) == -1) {
			syslog(LOG_ERR, "%s:ioctl(SIOCGENITER): ",
		...
		if ((eport == ntohs(ipn.in_pmin)) &&
			...
			if (r == -1)
				syslog(LOG_ERR, "%s:ioctl(SIOCRMNAT): ",
				       "delete_redirect_rule");
			return r;
		}
	} while (ipn.in_next != NULL);
				 ...
				 int proto, const char * desc)
{
	...
	if (r == -1) {
		if (errno == ESRCH) {
			syslog(LOG_ERR,
			       "SIOCINAFR(missing 'head %s' rule?):",
			       group_name);
		} else {
			syslog(LOG_ERR, "SIOCINAFR:");
		}
	}
	return r;
}
...
delete_filter_rule(const char * ifname, unsigned short eport, int proto)
{
	...
	if (ioctl(dev, SIOCGETFS, &wobj) == -1) {
		syslog(LOG_ERR, "ioctl(SIOCGETFS): ");
		goto error;
	}
	...
	do {
		...
		if (ioctl(dev_ipl, SIOCIPFITER, &wobj) == -1) {
			syslog(LOG_ERR, "ioctl(SIOCIPFITER): ");
			goto error;
		}
		...
		if ((fp->fr_type == FR_T_IPF) &&
			...
			if (ioctl(dev_ipl, SIOCRMAFR, &dobj) == -1) {
				syslog(LOG_ERR, "ioctl(SIOCRMAFR): ");
				goto error;
			} else {
				return 0;
			}
