/* $Id: miniupnpd.c,v 1.22 2016/11/28 01:12:51 bcm5357 Exp $ */
/* MiniUPnP project
...
#ifdef USE_DUAL_WAN
		system("/sbin/iptables -t nat -F preroute_upnp2");
#endif
#else
		system("/sbin/iptables -t nat -F upnp_in_public_preroute");
#ifdef USE_DUAL_WAN
		system("/sbin/iptables -t nat -F upnp_in_public_preroute2");
#endif
#endif
		system("/sbin/iptables -t filter -D INPUT -j upnp");
	}
...
OpenAndConfHTTPSocket(unsigned short port)
{
	...
	{
		syslog(LOG_ERR, "socket(http): ");
		return -1;
	}

	if(setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &i, sizeof(i)) < 0)
	{
		syslog(LOG_WARNING, "setsockopt(http, SO_REUSEADDR): ");
	}
...
OpenAndConfCtlUnixSocket(const char * path)
{
	...
	{
		syslog(LOG_ERR, "bind(sctl): ");
		close(s);
		s = -1;
	}
	else if(listen(s, 5) < 0)
	{
		syslog(LOG_ERR, "listen(sctl): ");
		close(s);
		s = -1;
	}
	return s;
...
set_startup_time(int sysuptime)
{
	...
	if(sysuptime)
	{
		...
		if(fd < 0)
		{
			syslog(LOG_ERR, "open(\"/proc/uptime\" : ");
		}
		else
		{
		...
		if(kc != NULL)
		{
			...
			if(ksp && (kstat_read(kc, ksp, NULL) != -1))
			{
				...
				syslog(LOG_ERR, "cannot open kstats for unix/0/system_misc: ");
			kstat_close(kc);
		}
#else
...
init(int argc, char * * argv, struct runtime_vars * v)
{
	...
	if(parselanaddr(&lan_addr[n_lan_addr], local_ip) == 0)
		n_lan_addr++;
...
shutdown:
	...
	{
		syslog(LOG_ERR, "Failed to remove pidfile %s: ", pidfilename);
	}
