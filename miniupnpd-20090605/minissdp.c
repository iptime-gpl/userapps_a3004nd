/* $Id: minissdp.c,v 1.3 2017/05/18 06:37:11 bcmac Exp $ */
/* MiniUPnP project
...
AddMulticastMembership(int s, in_addr_t ifaddr)
{
	...
	{
        syslog(LOG_ERR, "setsockopt(udp, IP_ADD_MEMBERSHIP): ");
		return -1;
    }
...
OpenAndConfSSDPNotifySocket(in_addr_t addr)
{
	...
	if( (s = socket(PF_INET, SOCK_DGRAM, 0)) < 0)
	{
		syslog(LOG_ERR, "socket(udp_notify): ");
		return -1;
	}
	...
	{
		syslog(LOG_ERR, "setsockopt(udp_notify, IP_MULTICAST_LOOP): ");
		close(s);
		return -1;
	}
	...
	{
		syslog(LOG_ERR, "setsockopt(udp_notify, IP_MULTICAST_IF): ");
		close(s);
		return -1;
	}
	...
	{
		syslog(LOG_ERR, "setsockopt(udp_notify, SO_BROADCAST): ");
		close(s);
		return -1;
	}
    ...
    if (bind(s, (struct sockaddr *)&sockname, sizeof(struct sockaddr_in)) < 0)
	{
		syslog(LOG_ERR, "bind(udp_notify): ");
		close(s);
		return -1;
    }
                 ...
                 unsigned int lifetime)
{
	...
	while(known_service_types[i])
	{
		...
		if(n < 0)
		{
			syslog(LOG_ERR, "sendto(udp_notify=%d, %s): ", s, host);
		}
		i++;
	}
}
...
SendSSDPGoodbye(int * sockets, int n_sockets)
{
	...
	for(j=0; j<n_sockets; j++)
	{
	    for(i=0; known_service_types[i]; i++)
	    {
			...
			if(n < 0)
			{
				syslog(LOG_ERR, "sendto(udp_shutdown=%d): ", sockets[j]);
				return -1;
			}
    	}
	}
...
SubmitServicesToMiniSSDPD(const char * host, unsigned short port) {
	...
	if(s < 0) {
		syslog(LOG_ERR, "socket(unix): ");
		return -1;
	}
	addr.sun_family = AF_UNIX;
	strncpy(addr.sun_path, minissdpdsocketpath, sizeof(addr.sun_path));
	if(connect(s, (struct sockaddr *)&addr, sizeof(struct sockaddr_un)) < 0) {
		syslog(LOG_ERR, "connect(\"%s\"): ", minissdpdsocketpath);
		return -1;
	}
	for(i = 0; known_service_types[i]; i++) {
		buffer[0] = 4;
